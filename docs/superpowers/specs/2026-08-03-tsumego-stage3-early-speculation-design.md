...
**正味＝analyse秒+generate秒**（root 解析の待ち＋着手決定＝ユーザーが実際に体感する
「白が打ってから黒が打つまで」の合計時間）を主指標として併記する。採用ゲートは
「root ウォール（analyse秒）非劣化 +0.3秒以内」で定義されていたが、これはそもそも
「ユーザー体感を悪化させないこと」の代理指標であり、正味秒がその意図を直接示す
（詳細は下の「判定」節）。

| ケース | 閾値 | 発火 | max_visits（3試行） | generate秒 平均（3試行） | analyse秒 平均（3試行） | 正味秒 平均（analyse+generate） |
|---|---|---|---|---|---|---|
| M@4 | 0.55（990v） | **0/3** | 870 / 880 / 879 | 3.56（3.47/3.84/3.37） | 1.80（1.82/1.77/1.82） | 5.36 |
| M@4 | 0.35（630v） | **3/3** | 866 / 1741 / 885 | 2.69（2.84/2.51/2.73） | 2.10（2.07/2.17/2.07） | 4.80 |
| O@2 | 0.55（990v） | 3/3 | 1633 / 1652 / 1657 | 0.35（0.19/0.47/0.39） | 2.24（2.27/2.22/2.22） | 2.59 |
| O@2 | 0.35（630v） | 3/3 | 1698 / 1527 / 702 | 0.10（0.07/0.09/0.15） | 2.15（2.12/2.27/2.07） | 2.26 |
| V2@2 | 0.55（990v） | **1/3** | 853 / 772 / **1233** | 3.88（3.78/4.16/3.71） | 2.19（1.87/2.07/2.62） | 6.07 |
| V2@2 | 0.35（630v） | **3/3** | 825 / 803 / 842 | 3.33（3.50/3.16/3.32） | 1.94（1.93/2.02/1.87） | 5.27 |

### Δ（0.35 − 0.55、独立試行平均・n=3同士）

| ケース | Δgenerate | Δanalyse（root ウォール） | **Δ正味（体感時間）** |
|---|---|---|---|
| M@4 | **-0.87s** | **+0.30s**（境界値。下の精密化で+0.20sに修正） | **-0.57s** |
| O@2 | -0.25s | -0.08s | **-0.33s** |
| V2@2 | **-0.56s** | -0.25s | **-0.80s** |

**3ケースとも正味は改善**（-0.57s / -0.33s / -0.80s）。判定はこの正味指標を主とする
（詳細は「判定」節）。

### M@4 analyse 精密化（追加3run、n=6）

M@4 の Δanalyse だけが唯一プラス（劣化方向）だったため、0.35 側のみ追加で独立試行3件
（`early_speculation_e2e.py M 4 1` をフォアグラウンドで3回、構成B）を測定し、既存3件と
合わせて n=6 で再集計した。

| # | analyse秒 | generate秒 | 発火 | max_visits |
|---|---|---|---|---|
| 1（既存） | 2.07 | 2.84 | FIRED | 866 |
| 2（既存） | 2.17 | 2.51 | FIRED | 1741 |
| 3（既存） | 2.07 | 2.73 | FIRED | 885 |
| 4（追加） | 1.87 | 2.63 | FIRED | 1008 |
| 5（追加） | 2.07 | 3.46 | FIRED | 959 |
| 6（追加） | 1.77 | 2.58 | FIRED | 1017 |

n=6 平均: analyse 2.00s／generate 2.79s／正味 4.80s。発火は **6/6**（全独立試行で発火、
0.35 の安定性がさらに裏付けられた）。

**訂正**: n=3 だけを見ると 0.55 側 analyse（1.77/1.82/1.82）と 0.35 側 analyse
（2.07/2.17/2.07）は完全に非重複だったが、**n=6 に拡張すると 0.35 側に 1.87s・1.77s が
加わり 0.55 側のレンジ（1.77〜1.82）と重なる**（サンプル4の1.77sは0.55側の最小値と同値）。
n=3 の非重複は小標本による見かけで、Δanalyse は **+0.30s ではなく +0.20s**
（n=6平均2.00s − n=3平均1.80s）に修正するのが実測に忠実。それでも 0.35 側の方が平均が
高い（1.80s→2.00s）傾向自体は消えておらず、方向としては「わずかな増加」が残る
（機序として、M@4 は発火率が 0/3→3/3〈さらに精密化後は6/6〉と最も大きく増えたケースで、
同時に発行される投機クエリが増えた分だけ root クエリが GPU を分け合う、という説明は
定性的には妥当）。ただし +0.3秒超の劣化ゲートには元々（n=3）から抵触しておらず、n=6 の
再測定でむしろ差は縮んでいる。

正味秒は n=3（4.80s）と n=6（4.80s）でほぼ一致し、analyse/generate の内訳配分が
サンプルごとに多少ぶれても合計はぶれない（正味が体感時間の指標として安定している
傍証）。
...
**主指標は正味秒（analyse+generate＝ユーザー体感時間）とする**。採用ゲート「root ウォール
非劣化 +0.3秒以内」は、そもそも「ユーザー体感を悪化させないこと」を測るための代理指標
だった。root ウォール（analyse秒）単体だけでなく、その後の着手決定（generate秒）を
含めた合計こそが実際にユーザーが待つ時間であり、正味秒はゲートの本来の意図をより直接に
表現する。

**発火率**: 0.55 で非飽和だった2ケース（M@4・V2@2）がともに 0.35 で 3/3（100%、M@4 は
精密化後 6/6）に到達（0.55 は M@4 0/3・V2@2 1/3）。O@2 は 0.55 の時点で既に 3/3 飽和して
おり、この閾値変更の効果を判別できるケースではない。

**正味秒（主指標）**: **3ケースとも改善**（M@4 -0.57s／O@2 -0.33s／V2@2 -0.80s）。特に
0.55 で非飽和だった2ケース（M@4・V2@2）は generate 短縮が単独でも 0.3秒ゲートを明確に
超えており、正味でも大きく改善している。O@2 は generate 単独では -0.25s と 0.3秒に届か
ないが、これは「0.55 でも既に発火して速かった」ケースでの上乗せ改善にすぎず、正味では
やはり改善（-0.33s）している。

**root ウォール（analyse秒）**: 3ケース中2ケース（O@2・V2@2）はむしろ改善（-0.08s・
-0.25s）。M@4 は当初のn=3測定で +0.30s だった。この値を「ノイズ域内」として片付けるのは
不正確 — n=3 の 0.55 側（1.77/1.82/1.82）と 0.35 側（2.07/2.17/2.07）は非重複で、
再現性のある方向性の差（M@4 は発火率が 0/3→3/3 と最も大きく増えたケースで、同時に発行
される投機クエリの増加分だけ root クエリが GPU を分け合う、という機序も定性的に整合する）
だった。ただし0.35側のみ追加で独立試行3件を測定しn=6に拡張したところ、0.35側に
1.87s・1.77sが加わり0.55側のレンジ（1.77〜1.82）と重なった。**Δanalyseは+0.30sではなく
+0.20s（n=6平均）に修正**するのが実測に忠実（詳細は上の「M@4 analyse 精密化」節）。
方向としての「わずかな増加」自体は n=6 でも残るが、いずれにせよ +0.3秒超の劣化ゲートには
一度も抵触していない。

**総合判定: 採用（`TSUMEGO_EARLY_SPECULATION_ROOT_FRACTION = 0.35`）**。主指標である
正味秒が3ケースとも改善（-0.57s／-0.33s／-0.80s）、root ウォール単体の劣化ゲート
（+0.3秒超）にも一度も抵触していない（M@4 の +0.20〜0.30s は境界値だが超過ではなく、
generate短縮がこれを上回って正味は改善）。定数のコメントに実測根拠を追記済み。
...
- **M@4 の analyse 増加は実効果だが小さく、正味では相殺されている**。n=3 では 0.55/0.35
  の両群が非重複で「明確な差」に見えたが、n=6 に拡張すると重なりが生じ Δanalyse は
  +0.30s→+0.20s に縮んだ。今後さらに閾値を下げる（0.35→より低い値）実験をするなら、
  同時発行される投機クエリ数がさらに増えて root ウォールへの影響が拡大する可能性があるため、
  analyse 秒は独立試行を複数件（n≥6目安）で計測するのが望ましい
