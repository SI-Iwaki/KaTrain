...
| O@2 | -0.25s | -0.08s | **-0.34s** |
| V2@2 | **-0.56s** | -0.25s | **-0.80s** |

**3ケースとも正味は改善**（-0.57s / -0.34s / -0.80s）。判定はこの正味指標を主とする
（詳細は「判定」節）。
...
**正味秒（主指標）**: **3ケースとも改善**（M@4 -0.57s／O@2 -0.34s／V2@2 -0.80s）。特に
...
正味秒が3ケースとも改善（-0.57s／-0.34s／-0.80s）、root ウォール単体の劣化ゲート
