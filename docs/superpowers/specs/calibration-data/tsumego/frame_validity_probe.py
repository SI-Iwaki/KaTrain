...
usage:
  python docs/superpowers/specs/calibration-data/tsumego/frame_validity_probe.py \
      <framed_sgf> <move_number> <xmin,xmax,ymin,ymax> <期待手csv> [trial_visits] [visits]

引数の SGF は**枠を張った後の盤**（キャプチャで実際に出題された盤＝保存SGFのroot）。
本体（コア）石はそこから復元する: リージョン内から「壁が乗っている辺」を除いたものがコアで、
どの辺が壁かは幾何だけでは決まらない（枠矩形が盤外にはみ出した辺には壁が置かれない）ため、
4辺の総当たり×(攻め方,コウダテ)4通りを再度枠張りして**元の盤に一致する組み合わせ**を採る。

出力:
  - 枠候補ごとの root lead / バランス距離 / 手番側の本体石 ownership（1子平均）
  - `frame_destroys_problem` の判定（+0.5/子 未満＝枠が詰碁を消している）
...
def trial(engine, board, region):
    ...
    engine.request_analysis(
        ...
        extra_settings=region_analysis_extra_settings(TRIAL_VISITS, 0.04),
    )
...
def main():
    ...
    framed = board_from_node(root, size)
    found = recover_core(framed, REGION, size)
    if found is None:
        print("core recovery FAILED: no wall-side subset reproduces the archived board")
        return
    ...
    try:
        ...
        for ko in (False, True):
            ...
            if any(board == prev for _, prev, _ in scored + destroyed):
                print(f"  frame ko={ko}: same board as the other candidate (no ko threat placed)")
                continue
            lead, ownership = trial(engine, board, region)
            ...
            dead = solver_own is not None and frame_destroys_problem(solver_own, len(pts))
            print(
                f"  frame ko={ko}: lead={lead:+.2f} dist={frame_balance_distance(lead):.2f} "
                f"solver_core={solver_own:+.2f}/{len(pts)} ({solver_own / max(1, len(pts)):+.2f}/stone) "
                f"-> {'DESTROYS the problem' if dead else 'usable'}"
            )
            (destroyed if dead else scored).append((ko, board, region))
        if scored:
            ko, board, region = scored[0] if len(scored) == 1 else (
                pick_balanced_frame([(k, b, r, trial(engine, b, r)[0]) for k, b, r in scored])[:3]
            )
        ...
        for tag, b, r in (
            ("frame    ", framed, REGION),
            ("frameless", core, katrain_region(frameless_region(core, 1), core)),
        ):
            try:
