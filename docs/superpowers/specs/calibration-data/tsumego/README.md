...
`D=4 / E=6 / F=2 / G=0 / G2=2 / H=4 / F2=4 / I=0 / J=10 / K=0 / L=0 / M=4 / N=0 / O=0 / P=2 / Q=0 / R=0 / S=0 / T=2 / U=0 / V=0 / V2=2 / W=0 / X=0`

**この表と正解手順は `e2e_suite.py` の `CASES` が持っている**（`expect` が対象手数と期待手、
`line` が初手から正解までの手順）。手で `generate_move_e2e.py` を叩く前にまずスイートを回す。
...
| `case-v-declass-no-kill-20260731.sgf` | **クラス裁定の格下げ先が「攻めないので何も起きず自明に clean」な手で、白が無条件で生きた誤答局面**（13路右上・枠あり・黒は攻め方、初手、正解 L12＝白 N10 → 黒 N13 でコウ／最終セキ＝白の無条件生きを防ぐ形、旧実装は K10）。region = `4,12,4,12`。**選択則もコウ検出も無実**（`frame_validity_probe.py`: 枠は健全 `black_attacks=True ko=False`・solver_core +0.90/子、`select_tsumego_move` は正解 L12 を選ぶ。コウ経路検査も L12=KO（応手 N10 の PV）/ K10=clean と正しく出る）。破れたのは格下げ先の妥当性で、case R と同じ穴だが**非解のほうが目数で優る**ため同着バンドでは塞げない（K10 pt-0.33 < L12 pt-0.29 で差 0.04 ＜ points_epsilon 0.25）。**役割石の絶対 ownership なら分離できる**（`class_screen_probe.py` 2run・同深さ800visits・相手石7子: L12 -0.99/-1.00 ・ K10 **-1.00/-1.00**＝どちらも ply1 では白を殺していない）のに対し、格下げが正しい4ケースの格下げ先は例外なく成立している（K C13 +0.99 / L J6 +0.99 / M K1 +0.98（守り方・自石8子）/ P J1 +0.99）＝**+0.98 と -1.00 の間に約 2.0 の空白**。`tsumego_declass_confirmed`（格下げ先を役割石の1子平均 >= `ko_success_ownership` で確かめてから差し替える。役割が読めない枠なし＝case R は従来どおり）の回帰対象（spec 追記31）。E2E: **L12 3/3**（旧実装は K10 3/3） |

| `case-v2-guard-outside-ko-20260731.sgf` | **クラス裁定に格上げ方向が無く、成立していない clean 手が採られて白が無条件で生きた誤答局面**（case V の続き＝黒L12 白N10 まで進めた局面、正解 N13＝コウ、旧実装は K10）。region = `4,12,4,12`、対象は **2手目まで進めた局面**（`generate_move_e2e.py ... 2 4,12,4,12`）。詰碁の順序で最下位なのは「相手が無条件で生きる」＝**失敗**で、成立していない clean 手はコウ手の**下**にいるのに、裁定は格下げ（コウ→無条件）しか持っていなかった。実測（同深さ800visits・役割石＝相手石8子・2run）: **どの手でも白は生きる**（K10 -0.91/-1.00 ・ L11 -0.99 ・ N13 -1.00 ・ L13 -1.00）ので目数・gain・ownership のどれも正解を指さず（正解 N13 は目数ガード best+2.0 の外で pt+7.97・v17）、分離できるのは**クラスだけ**（N13 のみ「応手 L11 の PV がコウ形に到達」2/2）。探す先は root policy（K10 .196 / L11 .0172 / **N13 .0133** / L13 .0021 / 残り全部 .00009＝NN 下限）。`tsumego_result_class` と `_ko_promotion_choice`（選択手が clean かつ役割石で失敗しているとき policy 上位を測って上位クラスへ格上げ）の回帰対象（spec 追記32）。**トリガーの誤爆は枠あり8ケースで確認済み**: 正解の clean 手は全部 ply1・800visits で成立している（D A4 +0.99 / E K1 +1.00 / J N10 +1.00 / K C13 +0.99 / L J6 +0.99 / M K1 +0.98 / P J1 +0.99 / T M1 +1.00）ので、それらの手番では root の movesOwnership で振るわれて**解析1本も走らない** |

| `case-w-frameless-declass-20260801.sgf` | **枠なし盤でクラス裁定の格下げ先を確かめておらず、コウの正解が「無条件だが失敗している手」に差し替わった誤答局面**（13路右下・**枠なし**・黒は守り方、初手、正解 H1→白 G1→黒 K1 でコウにして黒生き、旧実装は J1 で黒が無条件死）。region = `6,12,0,6`。**選択則もコウ検出も無実**（同深さ検証が正しく H1 を採り、コウ経路検査も H1=KO（応手 G1＝正解本譜の白の手）/ J1・G6・L1=clean と出る）。case V と同じ穴だが**枠なしなので `tsumego_declass_confirmed` が丸ごとスキップされて**いた（旧実装は `solver_attacks is None` で即 True）。case R の目数バンドも効かない — **格下げ先 J1 が目数最善**（J1 +1.94 < H1 +2.20）だから。実測（`class_screen_probe.py` 2run・同深さ800visits）: 自石7子 **H1 +0.51/+0.35 vs J1 -0.22/-0.21**、相手石9子 H1 -0.83 vs J1 -0.92 ＝ J1 は「clean だが黒が死ぬ」＝順序で最下位の失敗クラス。役割不明時の尺度は `tsumego_success_ownership` の**自石・相手石の1子平均の小さいほう**（J1 は -0.92）で、外し方が「格下げしない＝コウを維持」に倒れる。`tsumego_declass_confirmed` を役割不明でも走らせる修正の回帰対象（spec 追記33）。**この盤は2つの独立した誤答経路を持つ**（spec 追記33 / 34）:

1. **格下げ**（追記33、修正済み）: 上記のとおり枠なしで格下げ先を検証していなかった。
2. **gain 覆しの同深さ検証がトップ1指名だった**（追記34、修正済み）: 浅い L1(v95) の visit比が深さゲート境界 0.5 を run ごとにまたぎ（0.43〜0.58）、通った run では gain 首位が正解 H1 から L1 に入れ替わる。L1 が検証で却下されると incumbent が**次点の H1 ではなく目数最善 J1 に戻る**ため H1 が一度も測られない＝**GUI 実戦で J1 が再発した経路**。挑戦者を gain 降順の contenders 最大3手に広げて解決（`tsumego_score_best_challengers`。救済側が case F2 で学んだのと同じ構図）。

E2E: **H1 3/3・K1 3/3**（旧実装は初手 J1）。残る揺れは**救済経路**で、N4（自石 -0.21/子＝黒が死ぬ）を拾う run が 22run 中 5〜6回。救済採用手は目数ガード外なのでクラス裁定にかからない（case F2 対策）。**この残余は閾値では塞げない — 5案すべて実測で却下済み**（spec 追記36: 成立ゲート／`untilDepth=6`／visit比の床／目数の到達幅／検証 visits 増）。特に検証 visits を 800→2400 にすると値の幅は 3.3 → 0.8 に縮むが**収束先は H1 -11.0 < N4 -8.9 のまま**＝ノイズではなく尺度が逆で、`untilDepth=6`×2400 なら初手は正解が 6.0 差で安定する一方**同じ盤の3手目では J1(-8.5) が正解 K1(-10.3) を上回る**。case I / Q / R と同じ限界枠 |
...
```bash
python docs/superpowers/specs/calibration-data/tsumego/e2e_suite.py              # 回帰点だけ（既定）
...
python docs/superpowers/specs/calibration-data/tsumego/e2e_suite.py --all        # 既知限界(I/Q)も含める
```

ケース表（SGF・region・**正解手順**・回帰点）をスクリプト内に持ち、ケースごとに**別プロセスで
KataGo を起動**して `generate_move_e2e.py` を回し PASS/FAIL 表を出す（同一プロセスの再クエリは
NN キャッシュで独立サンプルにならない、という CLAUDE.md の注意を自動的に守る）。

**`line`（正解手順）と `expect`（回帰点）は別物**:

- `line` は正解手順（SGF root から交互に打つ GTP 列）。**SGF の本譜（`children[0]` 連鎖）は
  「実際に打たれた手順」＝誤答を含む線であることが多く、正解が分岐（variation）側にしか
  無いケースがある**（実測: D/F/L/O/T/U は分岐が正解、E/M/J は正解の継続が未記録）。
  スイートは局面をこの `line` から組む（`generate_move_e2e.py --line=...`）ので、
  本譜の誤答を経由せずに**初手から正解までの各黒番**を再現できる。
- `expect` は README / spec の追記に実測記録がある手番だけ。既定モードはここだけを PASS/FAIL する。
- `--full` は `line` の全黒番を回し、`expect` に無い手番の不一致を **DIFF** として報告する
  （＝「記録された手順と違う」であって即バグではない。アプリの解答樹に別解があるかを GUI で見る）。
...
```bash
python docs/superpowers/specs/calibration-data/tsumego/generate_move_e2e.py <sgf> <moves_csv> <xmin,xmax,ymin,ymax> [repeats] [--line=GTP,...]
...
select レベルの A/B に加えて必ずこれも回すこと**（通常は `e2e_suite.py` 経由で足りる）。

`--line` を渡すと SGF の着手を無視してその GTP 列を root から打ち直す。**SGF の本譜は
誤答を含む「実際に打たれた手順」であることが多く、`moves_csv` だけでは正解手順の途中局面に
到達できない**（正解が分岐側にあるケースがある）ため、手順の回帰にはこちらを使う。
...
**「両者を比べる」のは効かないが「格下げ先だけを絶対値で見る」のは効く**（spec 追記31）。
役割（攻め方＝相手石／守り方＝自石）の1子平均を、格下げ先について `ko_success_ownership`(0.5) と
比べる: 実測は 格下げが正しい4ケース（K C13 +0.99 / L J6 +0.99 / M K1 +0.98 / P J1 +0.99）と
case V の格下げ先 K10 **-1.00** の間に約 2.0 の空白。**役割ごとの列を見ること**（case M は
守り方なので相手石は -0.93＝生きているのが正常で、自石の +0.98 が成否を運ぶ）。
...
### `raw_role_probe.py` / `flip_role_probe.py` — 「役割は測って選べない」の却下実測（case X）

```bash
python docs/superpowers/specs/calibration-data/tsumego/raw_role_probe.py    # 生盤 ownership 案
python docs/superpowers/specs/calibration-data/tsumego/flip_role_probe.py   # 手番フリップ案
```

どちらも**役割の自動判定として実測で却下済み**（spec 追記37）。再検討するときはまずこれを回す:

- `raw_role_probe.py`: 保存 SGF から枠を剥がした**生盤（認識盤そのもの）**を全ケース分
  KataGo に読ませ、色別コア石の1子平均 aliveness から役割を導出して既知の正解役割と突き合わせる。
  実測: **殺す問題の白が死なない**（X +0.47 / S +0.32 / U +0.45＝生盤は外側が開いており「外側は
  攻め方の勢力圏」という詰碁の約束事を表現しない）。case G は黒 -0.67/白 +0.92 で**逆を確信**する
- `flip_role_probe.py`: 両役割×両コウダテの4枠それぞれで「想定守り方の群の生死が手番に依存するか」
