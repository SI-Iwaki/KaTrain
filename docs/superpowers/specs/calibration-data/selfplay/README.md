...
| `opponent_pool_9.json` | 9路の相手ボットの 3段位プール（`calibrate --size 9 --strategy enigma9plus:<実戦の多数派の 7 局の設定> --ranks rank_1d,rank_3d,rank_5d,rank_7d,rank_9d --games 8`・結果 `selfplay-calibration-results-20260926-9x9.md`） |
...
| `veil13-campaign.md` / `veil13-campaign/` | 韜晦（13路）の自己対局キャンペーン結果（計画 `2026-09-23-veil-strategy.md` の Task 15・16）。段階1（5アーム × 20局）・1b（つまみの掃引）・2（投了なし）・3（接戦ストレス）・3b はすべて 2026-09-24 に実施済み。主比較・罠 A/B・一致率の下限の内訳・採否・ユーザーの決定（13路の既定値は据え置き）。序盤の研究外し（spec §15・§16.4 の計測・2026-09-26）の `veil13-open-<段>-*` も含む。`veil13-campaign/` は段階1の `veil13-p1-run.json`・default 対各アームの summary（`veil13-p1-default-vs-<アーム>-summary.{json,txt}`）と、研究外しの `veil13-open-<段>-summary{,-vs-local}.{json,txt}`・`veil13-open-<段>-window_stats.json` などの写し |
| `veil9-campaign.md` / `veil9-campaign/` | 一致率ひかえめ9路の自己対局キャンペーン結果（計画 `2026-09-26-selfplay-9x9.md`。9路の校正と序盤の研究外しの計測＝spec §16・2026-09-26）。相手の校正（calib9）と 9-1〜9-5 の各段・実戦（`recon_9`・`recon_9_veil`）との比較。`veil9-campaign/` は各段の summary・window_stats・run.json の写し |
