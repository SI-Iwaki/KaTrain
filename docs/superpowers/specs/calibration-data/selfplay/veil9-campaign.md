...
| **対局中のレポート**（最後の `Rate:` の mine ＋最後の手番の chosen == best） | **155/250 = 62.0%** | **102/153 = 66.7%** | − |

- **ユーザーが見る一致率は KaTrain の対局中のレポート**（2026-09-26 にユーザーが確かめた）で、ハーネス（`selfplay_move_rows`）と同じ定義
  （対局中の解析の親局面の `candidate_moves[0]`）。ログの `Rate:` はこれと同じ数え方（最後の `Rate:` の mine が最後の手番の前までの Decision の chosen == best の数と一致）。
  **対局中のレポートでは layers で 66.7% → 62.0%（−4.7pt）に下がった**。ハーネスの予想（9-1 の spec − layers +9.8pt）の半分ほど。絶対値はハーネスより高い
  （前の設定 66.7%・ハーネスの spec 62.6%、layers 62.0%・ハーネス 52.8%）
- 事後 2500v（別の探索）では局平均の差 layers − 前の設定は +1.7pt（Welch の 95% 区間 [−7.2, +10.5]・9 局と 8 局）で下がって見えない。理由は次の項の同格の外し
- 手番の種類と事後の一致（`veil9-recon-layers-kinds.txt`）:
  ...
  事後 2500v では最善手だった（対局中の解析とは別の探索で、0.3 目以内の同格の手の順位が入れ替わる）。対局中のレポートの 62.0% と事後の 68.0% の差はほぼこれ。
...
- ハーネスの一致率は対局中のレポートと同じく、AI が判断に使ったのと同じ解析と比べるので、0.3 目以内の外しは必ず不一致に数える。ユーザーの目的（対局中のレポートの一致率）には
  この数え方が合っている。事後の別の探索で見ると、その一部が一致に数えられる（事後の数字は対局中のレポートより厳しい）
- 結果は 9-0-0（最終リード +7.4〜+59.0 目）

## 実戦（layers ＋ forced_max_loss 8 ＋ 研究外し 6・2026-09-26 夜）

ユーザーが設定を変えて打った一致率ひかえめ9路の実戦 3 局（18:36〜18:44 に始めた監視対局）。layers からの変更はログの `veil9_*` で 2 キーだけ:
`veil9_forced_max_loss` 5.0 → **8.0**、`veil9_open_moves` 0 → **6**（3 局とも途中で変わっていない）。一致率は対局中のレポートの数え方（最後の `Rate:` ＋最後の手番）。
窓の中は Decision の depth <= 12（ハーネスと実戦の窓の指標と同じ W）。最終リードは最後の Decision の lead（対局中の解析・AI 視点・最後の AI の手の前）。

| 局 | AI | 自分（レポート） | 相手（レポート） | 自分 窓の中 / 後 | 最終リード | lead < 3 の手番 | 手番の種類 |
|---|---|---|---|---|---|---|---|
| `20260926_183644` | W | 9/17 = 52.9% | 5/16 = 31.2% | 2/7 / 7/10 | +2.3 | 17/17 | best 9・opening 3・forced 3・free 1・swap 1 |
| `20260926_184059` | B | 16/40 = 40.0% | 25/39 = 64.1% | 3/7 / 13/33 | +1.9 | 29/40 | best 16・free 13・forced 6・swap 3・opening 2 |
| `20260926_184445` | W | 19/35 = 54.3% | 14/35 = 40.0% | 4/6 / 15/29 | +3.9 | 7/35 | best 19・free 11・opening 2・forced 2・paid 1 |
| **3 局** | | **44/92 = 47.8%** | 44/90 = 48.9% | 45.0% / 48.6% | +1.9〜+3.9 | 53/92 | |

同じ数え方の比較: 前の設定 8 局 66.7%（窓の中 / 後 66.7% / 66.7%）・layers 9 局 62.0%（50.8% / 65.6%）・この設定 3 局 47.8%（45.0% / 48.6%）。

- 下がったのは主に窓の後（65.6% → 48.6%）。研究外しの窓の手は 3 局で 9 手番（すべて `played`・窓の中の自分の手番の 20 のうち）
- 代わりに**接戦になった**: 最終リードは +1.9〜+3.9 目（前の設定 +5.6〜+40.0・layers +7.4〜+60.5）。`183644` は全手番で lead < 3、`184059` は 40 手番のうち 29。
  forced の層は打った後のリードを `forced_min_lead`（1.0）まで下げてよいので、上限を 8 目にすると 1 手で大きくリードを払う（`184059` に 7.9 目の forced の手＝レポートの「≥6」の手）
- free の外しが多い（3 局 25 手番＝27%・layers 14%）: lead < reserve の手番では払う外しができず、0.3 目以内の外しだけが残るため
- 3 局だけなので安全（負け・持碁）は言えない。9路のハーネスでは研究外し 12 手で強い相手（9-4）と接戦ストレス（9-5）の負けが増えた（上の各段）

## ハーネス: forced_max_loss 8 と研究外し 6 の分解（2026-09-26 夜）

上の実戦 3 局の設定（layers ＋ `forced_max_loss` 8 ＋ `open_moves` 6）を、ハーネスで 4 アームに分けて測った。git HEAD `38dbea6f`（dirty false）・設定の写しは
これまでと同じ `veil-open-config.json`（sha256 `8075cd05…`・`ai:enigma9plus` は今のユーザー設定と同じ＝任せる先の指紋 `90e4adbeb5877e1a`）。アームは
`veil9-campaign/veil9-f8-arms.sh`（layers の 25 キーから変えたキーだけ違う。`f8_open6` は今のユーザー設定の `ai:veil9` と 25 キーすべて同じことを確かめた）:
`layers`・`f8`（forced_max_loss 8）・`f8_open6`（＋open_moves 6）・`f8_open6_lead3`（＋forced_min_lead 3）。
段は 9-1（プール・24 対・seed 1000〜1023）・9-4（HumanStyle 9段・20 対）・9-5（HumanStyle 9段・`--komi-shift 2`・20 対）。
run は `experiments/selfplay/20260926_1856_veil9-f8-p1`・`20260926_2007_veil9-f8-p4strong`・`20260926_2107_veil9-f8-p5stress`（18:56〜22:01・256 局・aborted 0・
`WARN integrity` なし）。写しは `veil9-campaign/veil9-f8-<段>-{run.json,summary.json,summary.txt,paired-vs-layers.txt}`・手番の種類は `veil9-f8-kinds.txt`。

自分の一致率（ハーネス＝対局中のレポートと同じ数え方・局平均）と勝-負-持碁:

| アーム | 9-1 通常 | 9-4 強い相手 | 9-5 接戦ストレス |
|---|---|---|---|
| layers | 58.3%・24-0-0 | 57.7%・20-0-0 | 75.1%・14-4-2 |
| f8 | 54.2%・24-0-0 | 57.0%・18-0-2 | 70.9%・13-5-2 |
| f8_open6（実戦 3 局の設定） | 56.3%・24-0-0 | 60.5%・18-0-2 | 72.0%・12-3-5 |
| f8_open6_lead3 | 55.5%・24-0-0 | 64.9%・20-0-0 | 72.4%・14-2-4 |

同じ seed の対の差 − layers（自分の一致率・t 97.5% 区間）:

| アーム | 9-1 | 9-4 | 9-5 |
|---|---|---|---|
| f8 | −4.0pt [−10.0, +1.9] | −0.7pt [−10.1, +8.6] | −4.2pt [−12.4, +4.0] |
| f8_open6 | −2.0pt [−8.4, +4.4] | +2.8pt [−6.3, +11.9] | −3.0pt [−12.0, +5.9] |
| f8_open6_lead3 | −2.8pt [−10.1, +4.7] | **+7.2pt** [−0.6, +15.0]（boot +0.1〜+14.1・Wilcoxon p 0.042） | −2.7pt [−10.6, +5.2] |

- **どのアームも自分の一致率ははっきり下がらない**（区間はすべて 0 をまたぐ）。いちばん下がる向きが続くのは f8（−4.0・−0.7・−4.2pt）。研究外し 6 を足しても下がらない
- f8_open6 は flip が増える（9-1 +0.58/局 [+0.06, +1.10]・9-4 +0.50/局 [+0.13, +0.87]）。9-1 では ≥6目の手 +0.33/局（boot +0.04〜+0.67）・損失 +0.16 目/手
- 勝ち以外（負け＋持碁）: 9-4 は layers 0・f8 2・f8_open6 2・lead3 0、9-5 は layers 6・f8 7・f8_open6 8・lead3 6
- forced の上限 8 はめったに使われない: 5 目以上の forced の手は 9-1 で f8 3 手・f8_open6 9 手（24 局）、9-4・9-5 ではほぼ 0（リード − forced_min_lead が上限より小さい手番が多い）。
  forced_min_lead 3 は forced の層をほとんど止める（9-1 0.67・9-4 0.25・9-5 0.05 回/局。layers 2.08・2.60・0.80）＝9-4 の一致率が上がる
- 同じ設定・同じ seed の layers でも run によって揺れる: 9-1 は 58.3%（前の 9-1 は 52.8%）、9-5 は 14-4-2（前の 9-5 は 16-0-4）。24 局では ±5pt ほど動く
- 実戦 3 局の 47.8% は、ハーネスの f8_open6（9-1 56.3%・9-4 60.5%）より低い。実戦の layers 9 局も局ごとに 46.7〜73.9% と幅があり、3 局の差（局平均 −12.6pt）は
  偶然の範囲を出ない（Welch の 95% 区間は 0 をまたぐ）
