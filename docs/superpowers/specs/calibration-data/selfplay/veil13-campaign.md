...
- この層の手番（`new`）:
  ...
  - 接戦ストレス: 結末は gate 623・no_cand 314・played 147・rejected 58（来ない手番 486）。打った手 147 手（中央値 2 手/局・最大 16）のうちリード 5 目未満で 74 手。
    ...
    負けは 0 局（接戦ストレスはコミを AI に 4 目不利にずらしてあるので、cur・new とも全局で序盤の AI の勝率が 0.03〜0.09 まで下がり、全局を逆転して勝った）。
...
- 最善手しか無い手番の外しの校正（上の「計測」）の結果を見て、**ユーザーのローカル設定の 13路で ON にした**（forced の値は §14.2 の 13路の既定・失着の層も ON のまま・9路／19路は OFF）。コードの既定は OFF のまま。

## 序盤の研究外し（spec §15・§16.4）の計測（2026-09-26）

計画 `docs/superpowers/plans/2026-09-26-selfplay-9x9.md` の C8・spec §16.4。9路の段階（9-1〜9-5）と相手の校正・実戦の比較は `veil9-campaign.md`。
13路のローカル設定（`local`＝loose ＋ 失着 ON ＋ forced ON）に序盤の研究外し（`veil13_open_moves` 30＝両者の通算 30 手目までを難解＋13路に任せる）を足した
`local_open` を比べた。数値は `veil13-campaign/` に写した `veil13-open-<段>-summary-vs-local.{json,txt}`（run の要約＝先頭のアーム local と各アームの差）・
`veil13-open-<段>-summary.{json,txt}`（`summarize --compare local_open local`）・`veil13-open-<段>-window_stats.json`（W 30）と、実行ディレクトリの games.jsonl・moves.jsonl から取った。
対の差の信頼度は 0.975（ハーネスの既定）。

### 実行条件

- コード: git HEAD `0a6f0bfe`（`0a6f0bfed27bf655acb7fbe8fbb6def3f20e8e0d`）・dirty false（4 段の run.json）。エンジン・hp 監査は 9路の段と同じ（KataGo v1.18.2 CUDA・
  `b10c384h6nbttflrs.bin.gz`・humanSL `b18c384nbt-humanv0.bin.gz`・max_visits 2500・hp 監査 `rank_9d`）。13路・コミ 7.0・中国ルール・投了モデル length（`recon/` の実戦 18 局）
- 設定の写し `experiments/selfplay/veil-open-config.json`（9路と同じ・sha256 `8075cd05…`）を `--config` で渡した。`local` の 24 キーは写しの `ai:veil13` と同じ
  （＝ユーザーのローカル 13路の設定。写しに `veil13_open_moves` のキーは無い＝0）
- アーム（`experiments/selfplay/veil-open-arms.sh`）:
...
- 指紋（run.json）: local `5c1775e11cce1180`・local_open `20d53f604d6c9a7b`・enigma `0df30552dca4ec6e`。local_open の任せる先（`delegate`）は写しの `ai:enigma13plus`
  （user config・指紋 `0df30552dca4ec6e`＝enigma アームと同じ・段階1〜3 の enigma とも同じ）:

```json
{"enigma13plus_max_loss": 3.0, "enigma13plus_large_lead_max_loss": 10.0, "enigma13plus_min_winrate": 0.25, "enigma13plus_net_margin": 0.0, "enigma13plus_target_score": 2.0, "enigma13plus_aim_jigo": false, "enigma13plus_endgame_move": 75, "enigma13plus_unsettled_max": 16, "enigma13plus_locality_stddev": 2.0, "enigma13plus_locality_slack": 0.3, "enigma13plus_min_delta_e": 0.2, "enigma13plus_cheap_loss": 0.3, "enigma13plus_probe_extra": 6.0, "enigma13plus_opening_humanstyle_moves": 11.0, "enigma13plus_gamble_until_move": 30.0, "enigma13plus_gamble_min_winrate": 0.3, "enigma13plus_gamble_min_delta_e": 0.5, "enigma13plus_overdraft_deficit": 2.0, "enigma13plus_overdraft_answered_max": 99.0, "enigma13plus_overdraft_min_fooled_lead": 1.0, "enigma13plus_overdraft_probes": 6}
```

  `opening_humanstyle_moves` 11 により、窓の最初の 11 手は難解＋の中で HumanStyle 9段の手（spec §15.2）＝研究外しが始まるのは 12 手目から
- 相手: 13-1・13-2 は `opponent_pool_13.json`（rank_1k / rank_1d / rank_3d・τ 1.0）、13-2 はそれを定跡を知る相手で包んだ（`--book-moves 40`・`--book-loss` 0.3）。
  13-3 は HumanStyle 9段（コミ互角）、13-4 は段階3b と同じ接戦ストレス（`--komi-shift 4`・HumanStyle 9段）。seed は 13-1・13-2 が 1000〜1023、13-3・13-4 が 1000〜1019（全アームが同じ seed）

| 段 | ラベル・出力ディレクトリ | 局数 | 所要（開始は run.json の `created`、終了は games.jsonl の最終更新） |
|---|---|---|---|
| 13-1 効果 | `veil13-open-p1`（`experiments/selfplay/20260926_1149_veil13-open-p1`） | 48（2 アーム × 24 seed）・aborted 0 | 11:49〜12:41（約52分）。対局時間の合計は local 23.7分・local_open 27.5分 |
| 13-2 定跡を知る相手 | `veil13-open-p2book`（`experiments/selfplay/20260926_1241_veil13-open-p2book`） | 72（3 アーム × 24 seed）・aborted 0 | 12:41〜14:00（約1時間19分）。**最初は誤って切り離したプロセスで起動したので、1 局も終わる前に止め、同じディレクトリを `--resume` で再開した**（コンソール `veil13-open-p2book.console.txt` は再開後のもので先頭が `resume:` の行。局はすべて再開後に打った）。local 18.4分・local_open 25.1分・enigma 33.8分 |
| 13-3 強い相手 | `veil13-open-p3strong`（`experiments/selfplay/20260926_1401_veil13-open-p3strong`） | 60（3 アーム × 20 seed）・aborted 0 | 14:01〜15:15（約1時間14分）。local 20.3分・local_open 22.4分・enigma 31.1分 |
| 13-4 接戦ストレス | `veil13-open-p4stress`（`experiments/selfplay/20260926_1516_veil13-open-p4stress`） | 40（2 アーム × 20 seed）・aborted 0 | 15:16〜15:57（約40分）。local 19.7分・local_open 20.3分 |

13路は合わせて約4時間7分（見込み約6時間）。

- 健全性: 4 段とも aborted 0・コンソールに `WARN integrity` の行なし・fallbacks 0・humansl_errors 0・hp_audit_errors 0・shadow_errors 0・visits_low 0・ledger_mismatch 0。
  lead < reserve での同値でない外し（`veil.nonfree_below_reserve`・local 14〜72・local_open 15〜38/段）は moves.jsonl の kind で数えると**すべて forced の層の手**
- 研究外しの窓の確かめ: local_open は 4 段とも `games_with_open` が全局・`open_missing_inside` 0・`open_outside` 0・invariant 0・errors 0・not_in_cands 0。
  関門（`open = "gate"`・p_match が床 0.15 以下で任せない）が 13-1 で 1・13-3 で 3・13-4 で 2 手番（窓 30 手は長いので床の関門が効く）、ほかはすべて `played`

### 結果のまとめ

| 段 | 相手 | 勝-負-持碁 local / local_open / enigma | 自分の一致率 local / local_open / enigma | local_open − local の自分の一致率 [97.5% t 区間]（Wilcoxon p） | 窓の中 / 後の差（own_in / own_out） | 窓の中の相手の一致率の差（opp_in） | W 30 手目のリードの差 | flip/局 local / local_open |
|---|---|---|---|---|---|---|---|---|
| 13-1 | プール | 24-0-0 / 24-0-0 / — | 40.6% / 35.6% / — | **−5.0pt** [−9.8, −0.3]（0.018） | −6.1pt / −3.1pt | −13.6pt（p 0.0018） | −2.81 目（p 0.012） | 0.08 / 1.54 |
| 13-2 | プール＋定跡 40 | 24-0-0 / 24-0-0 / 24-0-0 | 49.7% / 42.6% / 50.0% | **−7.2pt** [−13.3, −1.0]（0.015） | −6.4pt / **−9.9pt**（p 0.014） | **−35.3pt**（p <0.0001） | **+2.65 目**（p 0.004） | 0.67 / 1.12 |
| 13-3 | HumanStyle 9段 | 20-0-0 / 20-0-0 / 15-2-3 | 47.5% / 50.7% / 64.4% | +3.2pt [−2.6, +9.0]（0.21） | −3.7pt / +7.6pt | −8.7pt（p 0.043） | **−1.55 目**（p 0.011） | 0.00 / 1.95 |
| 13-4 | 接戦ストレス | 18-2-0 / 18-2-0 / — | 62.7% / 62.3% / — | −0.4pt [−6.0, +5.1]（0.99） | **−8.0pt**（p 0.007） / +3.8pt | +0.7pt | −0.99 目（p 0.070） | 0.25 / 0.40 |
...
|---|---|---|---|---|---|---|---|---|---|---|
| local | 24-0-0 [86.2, 100.0] | 40.6% [37.3, 44.1] | 30.3% | 0.61 / 1.43 | 0.83 | 0.08 | +33.6 | 82.5 | 27.2%（621） | 0.56 |
| local_open | 24-0-0 [86.2, 100.0] | 35.6% [33.3, 38.0] | 24.1% | 0.62 / 1.29 | 0.83 | 1.54 | +25.7 | 82.5 | 19.6%（670） | 1.82 |
...
|---|---|---|---|---|---|---|---|---|---|---|
| local | 24 | 44.2% / 40.0% | 35.3% / 25.1% | 0.29 / 0.80 | 0.72 / 1.83 | 0.62 / 0.04 | +7.4 [+4.7, +9.3]・+2.1・0/24 | 0.08 / 0.00 | 0.75 | 8.04 |
| local_open | 24 | 38.1% / 37.0% | 21.7% / 26.6% | 0.52 / 0.70 | 0.78 / 1.65 | 0.96 / 0.04 | +3.6 [+1.7, +5.1]・+0.3・0/24 | 1.54 / 0.00 | 0.64 | 6.88 |

- 対の差（local_open − local）: own_top1 −5.0pt [t −9.8, −0.3 / bootstrap −9.6, −0.9]・opp_top1 −6.2pt [−14.1, +1.6]・flip +1.46 [+0.71, +2.21]（p 0.0006）・損失 +0.01・≥6目 0。
  窓: own_in −6.1pt [−14.9, +2.7]・own_out −3.1pt [−8.5, +2.4]・opp_in −13.6pt [−21.8, −5.4]・opp_out +1.4pt・lead_at_w −2.81 目 [−5.91, +0.29]
- 窓の手（local_open・24 局）: 任せた手番 359（ほかに関門 1）・kind opening 223・レポートの損失の合計 186.9 目（1手番 0.52 目）・≥2目 23 手・≥6目 1 手
- local の 40.6% は §14 の計測の `new`（同じ設定・通常の相手 38.5%）と同じ水準

### 13-2 定跡を知る相手（プール＋`--book-moves 40`・24 対）

定跡の出口（`window_stats` の `book_exit`）:

| アーム | 出口の手数 中央値（最小） | 出口の理由（AI の損失 `ai_loss` / 上限 40 `limit`） | 相手が定跡で打った手/局 | 雑音の出口（出口の手が実は最善手） | 出口の手の損失 中央値 / 平均 |
|---|---|---|---|---|---|
| local | 35.5（19） | 15 / 9 | 15.7 | 7/15 | 0.50 / 0.60 目 |
| local_open | 13（4） | 24 / 0 | 6.6 | 1/24 | 0.48 / 0.76 目 |
| enigma | 13（4） | 23 / 1 | 7.3 | 0/23 | 0.64 / 0.73 目 |

| アーム | 勝-負-持碁 [勝ちの Wilson 95%] | 自分の一致率 局平均 [95%] | 相手の一致率 局平均 | 自分 / 相手の損失（目/手） | ≥6目/局 | flip/局 | 最終リードの中央値 | 手数の中央値 | 外した手の 9段 hp 中央値（手数） | 戦略時間 p95（秒） |
|---|---|---|---|---|---|---|---|---|---|---|
| local | 24-0-0 [86.2, 100.0] | 49.7% [45.8, 53.7] | 56.0% | 0.47 / 1.31 | 0.79 | 0.67 | +29.4 | 82.5 | 25.7%（535） | 0.51 |
| local_open | 24-0-0 [86.2, 100.0] | 42.6% [39.5, 45.7] | 38.6% | 0.67 / 1.34 | 1.12 | 1.12 | +25.9 | 82.5 | 20.3%（601） | 1.71 |
| enigma | 24-0-0 [86.2, 100.0] | 50.0% [46.5, 53.5] | 35.3% | 1.41 / 1.93 | 3.83 | 3.04 | +10.2 | 83 | 3.4%（531） | 2.17 |

| アーム | 局 | 自分 窓の中 / 後 | 相手 窓の中 / 後 | 自分の損失 窓の中 / 後（目/手） | 相手の損失 窓の中 / 後 | 窓の中の ≥2目 / ≥6目（/局） | W 手目のリード 中央値 [IQR]・最小・負の局 | flip 窓の中 / 後（/局） | 窓の後の u の平均 | 窓の後の paid/局 |
|---|---|---|---|---|---|---|---|---|---|---|
| local | 24 | 53.9% / 47.5% | 92.5% / 36.3% | 0.09 / 0.64 | 0.14 / 1.96 | 0.08 / 0.00 | +0.1 [-0.4, +0.8]・-2.6・10/24 | 0.58 / 0.08 | 0.97 | 5.58 |
| local_open | 24 | 47.5% / 37.7% | 57.2% / 26.9% | 0.51 / 0.72 | 0.70 / 1.63 | 1.25 / 0.25 | +2.8 [+0.5, +4.8]・-2.3・3/24 | 1.08 / 0.04 | 0.87 | 8.58 |
| enigma | 24 | 45.6% / 51.6% | 57.2% / 22.4% | 0.61 / 1.77 | 0.80 / 2.55 | 1.62 / 0.21 | +1.8 [-0.5, +5.8]・-3.6・8/24 | 1.46 / 1.58 | — | 0.00 |

- 対の差（local_open − local）: own_top1 −7.2pt [t −13.3, −1.0]・opp_top1 **−17.4pt** [−25.9, −9.0]（判定 lower）・flip +0.46 [−0.48, +1.39]・損失 +0.21 [−0.03, +0.44]・≥6目 +0.33 [−0.48, +1.15]。
  窓: own_in −6.4pt・own_out −9.9pt [−18.9, −0.8]・opp_in −35.3pt [−45.2, −25.3]・opp_out −9.3pt [−19.3, +0.6]・lead_at_w +2.65 目 [+0.50, +4.80]
- 対の差（local − enigma）: own_top1 −0.3pt [−7.2, +6.6]・opp_top1 +20.7pt・flip −2.38・≥6目 −3.04
- 窓の手（local_open）: 任せた手番 360・kind opening 189・レポートの損失の合計 183.2 目・≥2目 30 手・≥6目 6 手
- 定跡の相手には local の窓の中で相手が最善手を続ける（相手の一致率 92.5%・W 手目のリード +0.1 目）。local_open は窓の中で定跡を外し（出口の手数の中央値 13）、
  相手が崩れてリードが +2.8 目になり、窓の後も lead < reserve（5 目）の手番が local 32.0% → local_open 13.3% と少ない（moves.jsonl の `decision_lead`・`decision_reserve`）ので、
  支払う外しが増えて（paid/局 5.58 → 8.58）窓の後の一致率も下がった（−9.9pt）

### 13-3 強い相手（HumanStyle 9段・コミ互角・20 対）

| アーム | 勝-負-持碁 [勝ちの Wilson 95%] | 自分の一致率 局平均 [95%] | 相手の一致率 局平均 | 自分 / 相手の損失（目/手） | ≥6目/局 | flip/局 | 最終リードの中央値 | 手数の中央値 | 外した手の 9段 hp 中央値（手数） | 戦略時間 p95（秒） |
|---|---|---|---|---|---|---|---|---|---|---|
| local | 20-0-0 [83.9, 100.0] | 47.5% [43.8, 51.4] | 44.5% | 0.33 / 0.52 | 0.20 | 0.00 | +7.6 | 83.5 | 26.9%（452） | 0.46 |
| local_open | 20-0-0 [83.9, 100.0] | 50.7% [47.2, 54.1] | 38.7% | 0.45 / 0.64 | 0.35 | 1.95 | +7.7 | 82.5 | 19.1%（415） | 1.73 |
| enigma | 15-2-3 [53.1, 88.8] | 64.4% [59.7, 69.3] | 39.4% | 0.69 / 0.78 | 1.60 | 3.60 | +4.3 | 110.5 | 6.4%（369） | 1.89 |

| アーム | 局 | 自分 窓の中 / 後 | 相手 窓の中 / 後 | 自分の損失 窓の中 / 後（目/手） | 相手の損失 窓の中 / 後 | 窓の中の ≥2目 / ≥6目（/局） | W 手目のリード 中央値 [IQR]・最小・負の局 | flip 窓の中 / 後（/局） | 窓の後の u の平均 | 窓の後の paid/局 |
|---|---|---|---|---|---|---|---|---|---|---|
| local | 20 | 51.0% / 44.8% | 42.3% / 43.8% | 0.10 / 0.46 | 0.24 / 0.65 | 0.10 / 0.00 | +2.5 [+1.7, +3.0]・+0.5・0/20 | 0.00 / 0.00 | 0.98 | 4.15 |
| local_open | 20 | 47.3% / 52.4% | 33.7% / 42.1% | 0.32 / 0.50 | 0.38 / 0.74 | 0.70 / 0.00 | +1.0 [-0.5, +1.5]・-1.7・8/20 | 1.90 / 0.05 | 0.98 | 3.60 |
| enigma | 20 | 49.7% / 70.6% | 34.0% / 41.2% | 0.43 / 0.76 | 0.50 / 0.84 | 1.15 / 0.05 | +1.3 [-0.1, +2.1]・-3.0・5/20 | 1.40 / 2.20 | — | 0.00 |

- 対の差（local_open − local）: own_top1 +3.2pt [t −2.6, +9.0]・opp_top1 −5.8pt [−12.7, +1.1]・flip +1.95 [+0.93, +2.97]（p 0.0006）・損失 +0.12・≥6目 +0.15。
  窓: own_in −3.7pt・own_out +7.6pt [−2.6, +17.7]・opp_in −8.7pt・opp_out −1.7pt・lead_at_w −1.55 目 [−2.79, −0.32]
- 対の差（local − enigma）: own_top1 −16.9pt [−23.6, −10.2]・flip −3.60・win +0.25 [+0.01, +0.49]・≥6目 −1.40
- 窓の手（local_open）: 任せた手番 297（ほかに関門 3）・kind opening 158・レポートの損失の合計 96.4 目・≥2目 14 手・≥6目 0
- enigma の勝ちでない局: 負け seed 1014（B・−24.1 目）・1016（B・−2.1 目）、持碁 1001（W）・1002（B）・1011（W）（すべて double_pass）
- 窓の後は、W 手目のリードが小さい分（負の局 8/20）lead < reserve の手番が local 47.0% → local_open 54.0% に増え、最善手も 46.5% → 54.0% に増えた（9路の 48 対と同じ仕組み）

### 13-4 接戦ストレス（`--komi-shift 4`・HumanStyle 9段・20 対）

| アーム | 勝-負-持碁 [勝ちの Wilson 95%] | 自分の一致率 局平均 [95%] | 相手の一致率 局平均 | 自分 / 相手の損失（目/手） | ≥6目/局 | flip/局 | 最終リードの中央値 | 手数の中央値 | 外した手の 9段 hp 中央値（手数） | 戦略時間 p95（秒） |
|---|---|---|---|---|---|---|---|---|---|---|
| local | 18-2-0 [69.9, 97.2] | 62.7% [59.3, 66.0] | 40.5% | 0.32 / 0.55 | 0.35 | 0.25 | +6.0 | 82.5 | 27.3%（351） | 0.39 |
| local_open | 18-2-0 [69.9, 97.2] | 62.3% [58.2, 66.4] | 45.4% | 0.33 / 0.55 | 0.20 | 0.40 | +4.3 | 84 | 27.2%（350） | 0.59 |

| アーム | 局 | 自分 窓の中 / 後 | 相手 窓の中 / 後 | 自分の損失 窓の中 / 後（目/手） | 相手の損失 窓の中 / 後 | 窓の中の ≥2目 / ≥6目（/局） | W 手目のリード 中央値 [IQR]・最小・負の局 | flip 窓の中 / 後（/局） | 窓の後の u の平均 | 窓の後の paid/局 |
|---|---|---|---|---|---|---|---|---|---|---|
| local | 20 | 77.7% / 54.7% | 38.0% / 40.8% | 0.06 / 0.48 | 0.29 / 0.65 | 0.00 / 0.00 | -0.0 [-0.9, +0.7]・-3.6・10/20 | 0.05 / 0.20 | 1.00 | 2.85 |
| local_open | 20 | 69.7% / 58.5% | 38.7% / 48.5% | 0.18 / 0.41 | 0.36 / 0.61 | 0.15 / 0.05 | -1.1 [-2.6, -0.1]・-4.4・16/20 | 0.25 / 0.15 | 1.00 | 2.65 |

- 対の差（local_open − local）: own_top1 −0.4pt [t −6.0, +5.1]・opp_top1 +4.9pt [−0.8, +10.7]・flip +0.15 [−0.26, +0.56]・win 0 [−0.18, +0.18]・≥6目 −0.15。
  窓: own_in −8.0pt [−13.3, −2.7]・own_out +3.8pt・opp_in +0.7pt・opp_out +7.8pt [−0.4, +15.9]・lead_at_w −0.99 目 [−2.42, +0.45]
- 窓の手（local_open）: 任せた手番 298（ほかに関門 2）・kind opening 90（AI 不利の局面では難解＋が最善手を選ぶ手番が多い）・レポートの損失の合計 53.4 目・≥2目 3 手・≥6目 1 手
- **両アームとも 2 局を落とした（どちらも 18-2-0）**。local はユーザーの今のローカル 13路の設定（loose ＋ 失着 ＋ forced）そのもので、§14 の計測の接戦ストレス
  （`new`＝同じ設定・同じ条件・40 seed）の 40-0-0 と違う（コードは研究外しの層を足した後・層は OFF。同じ設定の2 run を合わせると 60 局で 58-2-0。seed 1000〜1019 は両方の run にある）。
  負けた局（games.jsonl。原因は moves.jsonl の AI の手ごとの `points_lost` と `lead_before_ai` / `lead_after_ai`＝AI 視点から読んだ）:
  - local seed 1006（黒・168 手・−9.7 目）: 81・83 手目に forced の層の手が続き（レポートの損失 5.3・9.9 目）、リード +8.3 → +2.9 → −5.2 目・勝率 0.93 → 0.23 に逆転
  - local seed 1015（白・138 手・−1.3 目）: 50・54 手目の支払う外し（4.3・4.0 目・リード +11 → +6.4 目）と 58・60 手目の forced（2.1・1.1 目）で +2.8 目まで減り、
    64 手目の最善手（判定時の最善手・レポートでは 7.6 目の損）で +0.9 → −6.6 目
  - local_open seed 1006（黒・159 手・−11.7 目）: 53〜67 手目に自分の損（最善手 2.1・paid 1.4・forced 1.9 目ほか）と相手の好手が重なって +10.6 → −0.6 目、
    69 手目の最善手（レポートでは 5.3 目の損）で −9.3 目（窓の後）
  - local_open seed 1007（白・123 手・−17.8 目）: 一度もリードしていない（`lead_max` −3.0 目）。窓の中は 8 手目の 1.4 目のほかはほぼ最善手で、36〜42 手目の最善手でも
    レポートでは 2.1・1.8・3.1 目ずつ離された。同じ seed の local は勝ち（+3.9 目）
- 窓の中の外しでリードを失った局は無い（local_open の 2 敗は窓の後の損と、窓の最初から不利で一度もリードしていない局）。local の 2 敗のうち 1 局（1006）は forced の層の 2 手、もう 1 局（1015）は
  支払う外し・forced と最善手の読み違いが重なった

### 読み方（13路）

- **通常の相手（13-1）では研究外しで自分の一致率が下がる**: −5.0pt [t −9.8, −0.3]（40.6% → 35.6%）。窓の中が −6.1pt・窓の後も −3.1pt で、9路（窓の後で打ち消し以上）と違って窓の後に打ち消されない
  （W 手目のリードが 2.8 目小さくなっても +3.6 目残り、lead < reserve の手番は 6.6% → 13.3% にとどまる）
- 定跡を知る相手（13-2）では、窓の中の相手の一致率が 92.5% → 57.2%（−35.3pt）と大きく下がり、自分の一致率も −7.2pt。相手が定跡を外れて崩れるのでリードが増える（W 手目 +2.65 目）
- 強い相手（13-3）では下がらない（+3.2pt・n.s.）。W 手目のリードが −1.55 目で負の局が 8/20（local 0）、窓の後は lead < reserve の手番が増えて最善手が増える
- 安全: 勝ち以外は 13-1〜13-3 で両アーム 0（enigma は 13-3 で 5）、13-4 は両アーム 18-2-0（負けの内訳は上）。flip は local_open が 13-1〜13-3 で 1.1〜2.0/局（ほぼ窓の中）・13-4 で 0.40/局で、どの段も local より多い
- spec §16.5 の目安（13路は 13-1 の局全体の差が 0 をまたがずに下がり、13-2・13-3 で local_open の勝ち以外が enigma 以下）: 13-1 −5.0pt [−9.8, −0.3]・勝ち以外 13-2 0 ≤ 0・13-3 0 ≤ 5 → **満たす**。
  採るかどうかはユーザーが決める（計画 C10）

### ユーザーの決定（2026-09-26）

- **13路のローカル設定で研究外しを ON にする**（`veil13_open_moves` 30・任せる先は難解＋13路のユーザー設定＝最初の 11 手は HumanStyle 9段）。
  ほかのキー（loose・失着 ON・forced ON）は今のまま。コードの既定は OFF のまま。
- 13-4 接戦ストレスで今のローカル設定が 18-2-0 だったことは報告済み（forced の層の「20 局に 1 局ほどの負けは許す」の範囲として、設定は変えない）。
- 次は実戦で確かめる（各 10 局以上・ログの `Rate:` / `Decision:`〈`open`〉を集計）。
