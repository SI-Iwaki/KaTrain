...
## 追記（2026-07-31・実装後の実測）

`generate_move_e2e.py` に per-move 時間出力（analyse / generate）を追加して A/B 計測。
run1 の analyse はエンジン起動（約35秒）込み、run2/3 は同一プロセスの NN キャッシュが
効いたウォーム値（この 0.2 秒級のウォーム再解析が prefetch の効果の実証でもある）。

| case（経路） | 改修前 generate 秒（cold / warm） | 改修後（cold / warm） | 答え |
|---|---|---|---|
| M（screen+pool+格下げ確認） | 6.1 / 2.7〜2.8 | 5.1 / 2.2〜3.5 | K1 3/3 不変 |
| O（コウ脱出） | 6.5 / 1.8〜2.3 | 5.4 / 1.7〜1.9 | A11 3/3 不変 |
| V2（クラス格上げ） | 6.4 / 3.5〜4.0 | 4.7 / 2.5〜3.3 | N13 3/3 不変 |

E2E で測れるのは並列ファンアウト＋memo のぶんだけ（約 20〜25% 短縮）。先読み（prefetch）の
効果は「人間の考慮時間」がある GUI 対局でのみ現れる（的中時は白の応手後の 1800visits 解析が
約 2.5 秒 → 0.2〜0.5 秒）。枠採否判定の並列化はユニットテスト（read_batch 等価性3本）で
意味論保存を検証し、時間は GUI キャプチャの `枠の採否判定に X 秒` ログで確認する。

## 追記2（2026-08-01・レビューで見つかった prefetch の致命傷と修正）

差分レビュー（3レンズ並列）が「ownership なしの先読みは NN キャッシュを温めない」と指摘し、
実測で**本物**と確定した（`prefetch_cache_probe.py`: ownership なし先読み直後の実クエリ
2.70 秒＝コールド 2.69 秒と同一、ownership 付きウォーム対照は 0.28 秒。KataGo の NN
キャッシュは ownerMap の有無を区別する）。当初実装の `next_move` 指定は
`request_analysis` が includeOwnership を強制 OFF にするため、**使い捨ての複製ゲームで
応手を1手進めた子ノードを作って ownership=True で撃つ**方式に修正
（`Game._region_prefetch_sim`）。修正後の実測（`prefetch_cache_probe2.py`）: 先読み直後の
実クエリ **0.10 秒**。副次効果として terminate が先読みクエリだけに正確に当たるようになった
（本譜ノードの GUI 追加解析を巻き込まない）。あわせて (a) リージョン解除後の着手でも
先読み残骸を掃除（cancel を region 分岐の外へ）、(b) 投機的枠なし読みが枠採用キャプチャで
誤った ERROR 警告を出さないよう `_tsumego_frameless_board(quiet=True)` を追加。
