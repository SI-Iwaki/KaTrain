...
- `lead − ceiling < min(target, answered_max)`＝天井まで払えば帯に届く（§10。大差の手番で無駄なプローブを撃たない）
...
## 10. 実戦での確認（2026-09-18・難解＋13路・監視対局 3 局・`overdraft_deficit=2.5` ほか既定）

ログ `game_20260918_160943` / `162114` / `163443`（AI は黒・3 局とも AI 勝ち）。

| 局 | AI 手番 | 消費モード（窓） | 発動 | 内容 |
|---|---|---|---|---|
| 160943 | 45 | 30 | **4** | C6（lead +6.5 → 応じられたら −0.10／引っかかれば +2.64・find_hp 0.03）→ 相手 D6（正解は D5）＝引っかかって次の手番 +5.30。直後に K2（−0.03／+3.60）→ 相手 **C7＝正解**＝応じられて消費モードが切れ、以後は最善手と通常の外し（D4 vloss 0.35）。中盤に F10（−0.67／+3.88）→ 相手 A5（正解 M2）→ +7.19、直後に C11（−0.51／+3.60）→ 相手 H7（正解 M2）→ +7.15 |
| 162114 | 46 | 37 | 0 | リードが序盤から大差（終盤 +40〜74）で帯に届く手番が 10 しか無く、資格なし |
| 163443 | 42 | 29 | **1** | H11（lead +5.6 → −2.16／+2.79・find_hp 0.22・vloss 7.9）→ 相手 M11（正解は L12）。次の 2 手番は消費モード外（最善手）→ 3 手番後に勝率 83% |

- 5 回とも資格の条件（帯 [−2.5, 0)・find_hp <= 0.25・引っかかった後 >= +2）を満たし、`Over` 行の値と `played` 行が一致。
  相手の応手は 引っかかった 3・正しく応じた 1・判別不能 1。例外・エラー行は 0。
- 着手決定は 帯に届く窓の手番で中央値 1.5 秒（他の手番 0.2 秒）＝見込みどおり +1.2 秒前後。
- 発動は連続しうる（160943 の 11→12 手番目・26→27 手番目）。回数上限を付けない設計（§3-5）どおり。
- **見つけて直した非効率**: 窓の条件が「`over_cap > cap`」だけだったので、リードが天井（12 目）以上の大差の手番
  （3 局で 96 窓中 55）でも窓が開き、資格が原理的に出ない候補を 115 手プローブしていた（1 手番 +0.3〜0.5 秒。
  判定は変わらないが、上限内に収まった追加候補が通常の net 比較に混ざる副作用もあった）。
  `enigma9_overdraft_window` に帯の上端 `upper` を渡し、**`lead − ceiling >= upper`（天井まで払っても帯に届かない）なら窓を閉じる**
  ようにした＝大差の手番は OFF と同じ解析・同じ手。既定（upper 0・天井 12）ではリード 12 目未満の手番だけが対象。
