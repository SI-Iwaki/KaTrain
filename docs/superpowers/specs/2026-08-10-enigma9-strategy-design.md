...
## 追記10（2026-08-28）: 終局帯では難解さの選択をしない（9/13/19路共通）

ユーザー報告「ヨセも終わって終盤に pass でいいのに、難解のロジックが適用された余計な手を
打ち続ける。AI 予想手順（PV）の4〜5番目に pass が出ている＝pass が最善のはず。PV に pass が
出たらパスするのがベストでは」。

### 実測（改修前・9路・chinese・監視対局のログから SGF を復元）

`restore_sgf_from_log.py` は実ノード解析の priority を 1011 固定で探していたが、現行は
`PRIORITY_DEFAULT`(1000)+N（実測 1038/1039。追加 AI クエリは 10000+N、ponder/先読みは負）
なので復元できず、帯 1000..1999 に直した（AI 色の検算も `Move generation complete` の
末尾4手で行う＝ログ先頭に前の対局の着手が混ざっていても外れない）。

各手番の「KataGo 候補の pass の順位・損失（`relativePointsLost`）」「最善手の PV」
「humanSL 9d（8visits）の pass と盤上最上位」を並べた（`probe_pass.py`・使い捨て）:

| 局・手番 | lead | KataGo 最善 | PV | pass 順位/損失 | 9d pass / 盤上最上位 | 0目損の候補 | 旧実装の着手 |
|---|---|---|---|---|---|---|---|
| 064054 @57（白 AI） | +5.65 | J6 v196 | `J6 C5 J3 A7 G7 B8` | 3位 / **0.07** | 19.7% / 29.7% | **25** | H1（外し） |
| 065952 @72（黒 AI） | +73.6 | A3 | `A3 B8 A5 J8 H6 pass D1` | 2位 / −1.15 | 0.0% / 63% | 4 | B8 |
| 065952 @74 | +71.7 | A5 v1114 | `A5 pass A3 pass H6 pass A6` | 3位 / **124.6**（v1） | 0.0% / 60% | 2 | A5 |
| 065952 @80 | +72.3 | H6 v2011 | `H6 A1 A9 pass D1 pass A3 pass` | 2位 / **126.5**（v1） | 0.0% / **99.5%** | 1 | H6 |
| 065952 @82 | +73.0 | A9 v166 | `A9 A1 J8 pass D1 A3` | 17位 / 0.2 | 0.0% / 50.5% | 8 | A9（外し） |
| 065438 @71（白 AI） | +18.1 | J8 v2001 | `J8 C9 G9 B4 A9 B3 J1 A3` | 3位 / 55.0（v1） | 0.0% / 99.9% | 1 | J8 |

読み取り:

1. **ユーザーの前提「PV に pass ＝ pass が最善」は成り立たない**。PV の偶数手目（4番目・
   6番目）の pass は**相手の**パスで、「相手にはもう打つ手が無いが自分にはある」局面の
   指紋。065952 @74/@80 は PV に pass が3つ並ぶのに、自分が今パスすると 124〜126 目損
   （KataGo の chinese ルールは死石の取り込みが終わるまで死石を生きと数える＝lead +72 が
   −52 に反転する）。「PV に pass が出たらパス」は**この2手番でパスして逆転負け**する。
2. 本当の「余計な手」は 064054 @57 型: pass の損失 0.07 目・0 目損の候補 25 手＝盤上に
   0.5 目以上の手が無い帯なのに、難解さの採点が「最も難解な 0 目損の埋め手」H1 を選ぶ。
   ここで 9d は pass 19.7% に対し盤上の手 29.7%（同一局面の別プロセスでは J3 36.1%）＝
   「まだ埋める手（ダメ・ツギ・死石の取り込み）がある」と見ている。
3. 「pass の損失だけ」で決めると area scoring のダメ帯（2026-08-06: ダメ13個で 0.10 目）で
   誤発火し、「humanPolicy だけ」で決めると @80 のような pass 大損の局面で目数の裏付けなしに
   9d を信じることになる。**両方の AND**（他5戦略の `_area_scoring_should_pass` と同じ）。

### 設計

`_generate_move` のゲート1（`best_gtp == "pass"` → パス）の直後にゲート1b を追加。
**pass が KataGo 候補に居て損失 < `_AREA_PASS_MARGIN`(0.5)** の手番だけ走り、その手番は
難解さの採点（子局面プローブ）に入らない:

1. 相手が直前にパス（`cn.move.is_pass`） → パス。クエリ 0 本。
2. humanSL 9d を 8visits で1本（`_probe_children([], player, parent_hp=True)`＝プローブ
   バッチの親 humanSL と同一条件・子プローブ無し）。9d の最上位が pass
   （`enigma9_terminal_pass`＝`_pass_preferred`、同着は pass 側）→ パス。
3. 9d の盤上最上位が KataGo 候補で損失 < 0.5（`enigma9_terminal_move`）→ その手を打つ
   （ダメ・死石の取り込み・ツギ）。難解さでは選ばない。
4. それ以外（9d の手を KataGo が読んでいない／0.5 目以上損と読む、humanSL 不在）→ 最善手。

pass の損失が 0.5 以上の手番（序盤〜ヨセ本体）は解析条件も採用判断も**ビット同一**。
`_area_scoring_should_pass` の判定本体は `_pass_preferred` に切り出して共有（他5戦略は不変・
`tests/test_area_scoring_pass.py` 4件そのまま）。

### 実測（改修後・katrain_debug・同じ局面）

| 局・手番 | 改修後 | ログ |
|---|---|---|
| 064054 @57 | **J3**（旧 H1） | `Terminal: pass loses 0.08, humanSL prefers J3 36.1% over pass 19.9% -> play it (loss 0.03)`・着手決定 0.0 秒 |
| 065952 @82 | **J8**（旧 A9） | `Terminal: pass loses 0.37, humanSL prefers J8 52.8% over pass 0.0% -> play it (loss 0.18)` |
| 065952 @80 / 065438 @71 / 064054 @55 | 不変（最善手） | 帯の外（pass 損失 33〜126 目）＝`Pool: … admissible=0` の従来経路 |

### 既知の縁

- pass の損失は run 間で ±0.2 動く（同一局面で 0.07/0.08、0.20/0.37）。0.5 近傍の手番は
  run によって帯の内外が入れ替わるが、外れた run は従来の外し（0 目損の埋め手）に戻るだけ。
- 盤面監視モードではアプリ側のパスは盤が変わらないので注入されず、(1) は効かない。
  (2)(3) が終局まで運ぶ＝9d が pass を最上位に置く局面（ダメ・取り込みが済んだ後）でパス。
- 回帰: `tests/test_ai_enigma9.py` `TestTerminalHelpers`（9件）/ `TestTerminalEndToEnd`（8件）。

### 追記10-2: 相手のパス後は ownership で終局を判定する

上の規則だけで終局まで自動進行させると（enigma9 が両者を担当・`finish_game.py`）、接戦の
064054@57 は `J6 A7 pass A6 J3 B8 G7 pass pass` の9手で終局した（全手 0.0 秒）が、大差の
065952@82 は白が毎手パスする中で黒が死石を **25 手以上**かけて1子ずつ取り込み続けた。
KataGo の chinese ルールは未取り込みの死石を生きと数えるので pass の損失が 126 目に出て
規則 (1) の `< 0.5` を通らないため。

同じ局面の ownership（2000visits・wRN=0）は**全81点 |o| >= 0.97**（死石は黒地として決着済み）。
接戦側の相手パス直後も全点 1.00。そこで規則 (1) を2段にした:

1. 相手が直前にパス かつ pass の損失 < 0.5 → パス（クエリ0本・従来どおり）
2. 相手が直前にパス かつ 全点 |ownership| >= `ENIGMA9_SETTLED_ABS`(0.9) → パス
   （ownership クエリ1本＝ヨセ Probe と同条件）。終局すれば KaTrain の結果も ownership で
   数えられるので、この pass に実損は無い
3. 相手が直前にパス で未決着点あり（ダメ・本当の未決着）→ 最善手。惑わす相手はもう
   居ないので外しはしない

0.9 は実測の決着側の最小 0.97 と、未決着側（ダメ 0・セキ 0・生死不明の群 0.3〜0.7）の
間。厳しいほうへ外れると「相手のパス後に最善手を1手打つ」だけで、危険側には倒れない。
改修後の自動進行: 065952@82 は `J8 → 白 A1 → G8 → 白 D1 → F5 → 白 pass → 黒 pass`（白の
放り込みには応じ、白がパスした直後にパス・判定 1.1 秒）、064054@57 は `J6 → A7 → 白 pass →
黒 pass` の4手で終局。

盤面監視モードではアプリ側のパスが注入されない（盤が変わらない）ので、この規則は
KaTrain 側で相手のパスが入る経路（GUI 対局・監視中に手動でパスを入れた場合）でだけ効く。

## 追記11（2026-09-01）: own_rare の「応手自明」減衰ゲート（9/13/19路共通）

**ユーザー報告**（13路・白番・aim_jigo・`game_20260901_180139`）: 「相手を騙して罠にかける
戦略のはずなのに、36手目 B10 と 38手目 C11 への黒の応手は候補がほぼ1つで humanPolicy も
圧倒的＝どんな人間でも正解を応じられる手を打っている」。

**実測の内訳**（ログの Score 行そのまま）:

- **move 36 B10**: vloss=0.94・E=**0.03**・find_hp=**0.983**（9段の正解応手 A10 が 98.3%）・
  own_hp=0.003 → net 0.39 > 最善 B5 の 0.18。net の中身は own_rare 0.99 が全部
  （E ほぼゼロ・reply_rare は find >= `ENIGMA9_HP_BOOK`(0.25) で 0）＝**戦略自身が
  「相手はほぼ確実に正答し、罠の価値は 0.03 目」と算定した上で、自手の珍しさだけを
  理由に 0.94 目払った**。追記6（ヨセの own_rare 支配）と同じ構造の中盤版で、
  追記8 の局所性は距離軸なので B10（アンカーから距離5）は塞げるが設定 OFF だった。
- **move 38 C11**: vloss=0.42・E=**0.84**・find_hp=0.715 → net 1.45。own_rare を 0 に
  しても net 0.50 > B5 0.02 で採用される＝E 主導の期待値プラスの本物の罠（9段でも
  28.5% で C12 を外し、外すと平均約3目）。**こちらは設計どおり**。

**対処**: `enigma9_net_score` の own_rare 項に減衰係数 `enigma9_own_rare_find_gate(find_hp)`
を掛ける。**find_hp <= `ENIGMA9_OWN_RARE_FIND_FADE`(0.85) は 1.0＝ビット同一**、
0.85→1.0 で線形に 0（B10 の 0.983 では 0.11）。設計判断:

- **狙い撃てる軸は find_hp だけ**。net_margin を上げると正当な外し全部が締まり、
  E フロアを課すと序盤の定跡外し（E≈0 が意図的な帯・9路校正 move 2 F7）が死ぬ。
  reply_rare は 0.25 以上で 0 に張り付き「それ以上自明」を罰しないので、ここが唯一の穴。
- **ハード閾値でなく線形ランプ**: find_hp は humanSL の run 間分散 ±0.05〜0.09 で揺れる
  （TensorRT バッチ非決定性）ので、崖だと境界の候補の採否が run ごとに反転する。
- **E・reply_rare は触らない**: find が高くても E で勝つ罠（C11）はそのまま採る。
- 0.85 の位置: 既存の校正例は全部下側（9路序盤 F7 find 0.17 / 勝勢 F3 0.059 /
  ヨセ校正 0.828〜0.891 のうち net 判定に効いた帯は w_own=0 のヨセ）。この対局の
  全38白手番では find > 0.85 の候補は12件で、**採用された外し22手のうち該当は B10 のみ**。

**副作用（開示済み）**: (a) 最善手自身が find > 0.85 かつ own_hp < 0.25 の稀な形では
最善手の net も下がり外しがわずかに出やすくなる（この対局では該当ゼロ。方向は
「応手自明の手の難解さ過大評価の是正」で意図に沿う）。(b) find 0.85〜1.0 帯の候補は
find_hp の run 間揺れが net に伝わるようになる（従来この帯の揺れは無影響）。
どちらも「そもそも採用されるべきでない帯」の中の話。
...
## 追記12（2026-09-02）: 難解＋（13路）`ai:enigma13plus`＝ΔE 床つきの改良版を新戦略として追加

### 起点（ユーザーの問い）

「難解は相手を罠にかけて損をさせる戦略という認識。今のロジックがベストか、改善の余地があるか。
体感では難解（13路）が最も罠にかかる」。既存の難解（9/13/19路）は残したいので、改良は
**新戦略として追加**する（既存3戦略はビット同一）。

### 実測1: E は較正されている（13路・監視対局7局・154手番）

対象ログ `game_20260901_180139` 〜 `game_20260902_003240` の7局（AI 白4局・黒3局＝
003240/235425/185551 は AI 黒。ログ先頭が `Playing move` なら AI 黒）。KataGo を回さず、
`Spend: lead=…` / `Endgame budget: lead=…` 行の lead 差分から相手の実損失を逆算した:

    realized_opp_loss = lead(次の自手番) − lead(この手番) + vloss(この手番の選択手)

lead が記録されるのは勝勢/ヨセ手番なので互角帯は測れていない（356手番中 154）。

| 予測 E の帯 | n | 予測 E 平均 | 相手の実損失平均 |
|---|---|---|---|
| 0〜0.25 | 18 | 0.12 | −0.02 |
| 0.25〜0.75 | 25 | 0.45 | 0.88 |
| 0.75〜1.5 | 43 | 1.06 | 1.30 |
| 1.5〜3 | 46 | 2.15 | 2.35 |
| 3以上 | 22 | 4.99 | 5.28 |

OLS `realized ~ 1 + E + reply_rare + own_rare`（n=154）: E +1.09（t=5.1）、reply_rare −0.35
（t=−0.4＝E を与件にすると寄与ゼロ）、own_rare +1.37（t=2.0＝珍しい手ほど 9d 想定より相手が崩れる）。
R² は E 単独 0.18・rarity 込み 0.20＝1手単位の予測力は弱く、集計でだけ成立する較正。
humanSL rank_9d をこの相手集団に当てても E の水準はずれていない＝**プロファイル変更は優先度低**。
own_hp >= 0.25（本の手）の外しは E 2.14 なのに実損失 0.43（n=16）＝定跡形では相手は想定よりよく受ける。

### 実測2: 改善余地は「E をほぼ買わない外し」

外し 200 手のうち、選択手の E − 最善手の E（ΔE）が 0.2 未満の外しが **20 手**、そこに vloss 合計
**29.6 目（約 4 目/局）**を払って realized gain（相手の実損失 − 払った vloss）は **−0.52 目**。
ΔE >= 0.2 の 90 手は +0.79 目。最善手より E が**低い**のに 0.3 目超払った外しが 13 手・23.4 目。

機構: `net = E + reply_rare + own_rare − cost_weight·vloss` で own_rare は humanPolicy ≈ 0 の手に +1.0、
消費モードの cost_weight は 0.1〜0.6 まで下がるので 3.5 目の損失が net 上 1 目弱にしか見えない。
例（`game_20260902_003240` AI 黒 35手目・lead +7.43・cost_weight 0.27）: 最善 F6 は E 3.10・own_hp 0.989・
net 3.50、J5 は vloss 3.48・E 2.65・own_hp 0.000・net 3.57 → J5 採用（相手の実損失 +0.35）。
最善手のほうが罠として大きく無料だった。

### 設計: ΔE 床（案 B）

外し候補（挑戦者）を、選択関数 `enigma9_choose_local` に渡す前に次で絞る:

- **E − 最善手の E >= `min_delta_e`（既定 0.2）**、または **vloss <= `cheap_loss`（既定 0.3）**
- 残らなければ最善手。落とした候補は `DeltaE filter: dropped <gtp> (dE=… < …, vloss=… > …)` とログ

`cheap_loss` の免除が要るのは、序盤は E が全候補 0.05 前後で ΔE 床だけだと「定跡を外す」要件
（own_rare の存在理由）が死ぬから。安い外し（序盤の vloss 0.04〜0.18、最善手より良い手）は残る。

ログ 200 外しへの機械的再適用:

| | 案 A: ΔE>=0.2 必須 | **案 B: ΔE>=0.2 か vloss<=0.3（採用）** |
|---|---|---|
| 変わらない外し | 129 | 163 |
| 最善手に戻る | 54 | 15 |
| 別の挑戦者に変わる | 17 | 22 |
| 払う vloss 合計（7局） | 210 → 180 目 | 210 → 180 目 |
| 予測 E 合計 | 265.5 → 267.0 | 265.5 → 268.2 |
| 序盤（AI 20手目まで）で変わる数 | 44 / 97 | 19 / 97 |

案 A は序盤の外しを半分近く消し、AI 25手目のように E5（0.87 目）→ L7（2.84 目）と**より高い手へ流れる**
副作用もあった。案 B で変わる代表例: 003240@35 J5→F6（最善・E 3.10・0目）、190522@22 G6（5.93目・E 4.84）→
B8（最善・E 5.08・0目）、185551@18 H12（7.40目・E 2.82）→ H6（0.79目・E 1.75＝外しは維持）、
180139@18 B10→B5（追記11 の find ゲートで直した事例の一般化）。不変の例: 003240@1 L4（vloss 0.04）、
003240@38 E1（vloss −0.86）。

**体感の変化**: 勝勢時に「わざと数目損して珍しい手」を打つ頻度が下がり、lead が target へ収束するのが
遅くなる（消費モードの「余剰を難解さに換える」がやや弱まる＝勝ち幅が残る方向）。E で勝つ本物の罠・
互角の安い外し・ヨセ予算・勝率フロア・着手時間は不変。**変わらない手番はビット同一**なので、解析条件を
変える改修で起きるシャッフル（CLAUDE.md「やってはいけないこと」）は構造的に起きない。

### 実装

- 純関数 `enigma9_delta_e_filter(scored, best_gtp, min_delta_e, cheap_loss) -> (kept, dropped)`（ai.py）。
  順序保存・最善手は常に kept・`min_delta_e <= 0` は OFF（scored をそのまま返す）・最善手のエントリが
  無ければ何も落とさない。境界は eps=1e-9 で inclusive（1.2−1.0 が 0.2 を割る浮動小数の丸め対策）
- 基底 `Enigma9Strategy._filter_challengers(scored, best_gtp)` は no-op（`(scored, [])`）。`generate_move`
  は `enigma9_choose_local` の直前で `scored, _ = self._filter_challengers(scored, best_gtp)` を呼ぶだけ
  ＝既存3戦略の採用判断・解析条件はビット同一（`tests/test_ai_enigma_plus.py::TestFilterHook`）
- `Enigma13PlusStrategy(Enigma13Strategy)`: `KEY_PREFIX="enigma13plus"` / `LABEL="Enigma13Plus"` /
  `SETTING_DEFAULTS = {**Enigma13.SETTING_DEFAULTS, "min_delta_e": 0.2, "cheap_loss": 0.3}`
  （`ENIGMA9_MIN_DELTA_E` / `ENIGMA9_CHEAP_LOSS`）。`generate_move` は非オーバーライド。sticky ヨセフラグは
  `game._enigma13plus_endgame`
- 定数 `AI_ENIGMA_13_PLUS = "ai:enigma13plus"`（`AI_STRATEGIES_ENGINE` / `RECOMMENDED_ORDER` / `AI_STRENGTH`）、
  `AI_OPTION_VALUES`/`AI_OPTION_ORDER` に12項目（10項目は enigma13 と同じ候補値。`min_delta_e` は
  `(0.0,"OFF")/0.1/0.2/0.3/0.5`、`cheap_loss` は 0/0.2/0.3/0.5/1.0）
- config.json（パッケージ）は enigma13 の既定 + 0.2/0.3。ローカル config はユーザーの enigma13 現在値
  （max_loss 1.6・target_score 1.5）を写して同条件で比較できるようにした
- i18n en/jp: `ai:enigma13plus`（難解＋（13路）/ Enigma+ (13x13)）・`aihelp:enigma13plus`・12 ラベル
  （`[13路＋]` / `[13x13+]` 接頭辞）。`compile_mo` 済み
- `katrain_debug` CLI 名 `enigma13plus`

### 検証

- `tests/test_ai_enigma_plus.py`（純関数9・フック4・登録6・GUI/config/i18n 整合3）＋既存 enigma テスト
  ＋ `test_tsumego_player_setup` ＋ `test_ai_options_grid` ＝ 173 PASS
- katrain_debug リプレイ（復元 SGF `game_20260902_003240`・`--move 68`＝AI 黒 35手目・ユーザー設定
  max_loss 1.6 / target 1.5）: 下記「リプレイ結果」
- **未実施**: 実対局での realized 再計測（案 B で変わる手番の新しい選択手の実効果は「E が較正済み」を
  前提にした見込み）。測り方は実測1と同じ＝ログの lead 差分だけで KataGo 不要

### 留意点

- 閾値 0.2 / 0.3 は今回の分布から置いた仮置き。GUI から変更可・`min_delta_e` 0 で難解（13路）と同一
- 9路・19路版は未追加（同じ型＝サブクラス＋`_filter_challengers`＋設定2項目で足せる）
- 標本は同一アプリの相手・勝勢局面中心。互角帯の realized は lead が記録されないので未計測

### リプレイ結果（katrain_debug・復元 SGF `game_20260902_003240`・`--move 68`＝AI 黒 35手目・max_loss 1.6 / target 1.5）

- **難解＋ `enigma13plus`**: Spend lead=7.31 cost_weight=0.28。最善 F6 E=3.15 net 4.02、J5 vloss 2.79 E=2.84 net 3.98。
  `DeltaE filter: dropped J5 (dE=-0.31 < 0.20, vloss=2.79 > 0.30)` ほか E12/J3/K5/J6/J4 の計6手を落とし
  `8 -> 2 candidates` → `Best move wins the confusion race` → **F6**（0目）
- **難解 `enigma13`（同一局面・別プロセス）**: 最善 F6 E=2.98 net 3.23 に対し **K5 を採用**（vloss 4.80・E 2.83
  ＝最善手より E が低い・wr 93.1%）。実戦ログの J5（3.48目）とは run 間分散で外し先が違うが「最善手より E の
  低い手に 3〜5 目払う」構造は同じ
- 両 run とも Score 表の E・vloss は KataGo の run 間分散の範囲で実戦ログ（F6 E 3.10 / J5 vloss 3.48・E 2.65）と一致

## 追記12-2（2026-09-02）: 難解＋の罠探索の拡張 `probe_extra`（＋先読み wave2 の同期）

### 起点（ユーザーの再定義）

ΔE 床（追記12）は「罠の総量は据え置きで代金だけ減る」＝AI が少し強くなる方向で、ユーザーの意図
「**相手が最善手・AI 推奨手で応じるのが難しく、より大きい損失手を打つように誘導する力（騙しやすさ）
そのものを上げる**」とは違った。代金の許容は「勝勢の余剰だけ」（互角の cap 1.6・勝率フロアは不変）。

### 実測3: 罠の威力を縛っているのは「探す範囲」

同じ7局（352手番・probe 済み候補 1502）:

- 子局面プローブの挑戦者は `enigma9_shortlist` の**安い順 7 手**。admissible が 8 手を超える 118 手番で、
  プローブした挑戦者の最大 raw loss は **cap の 0.38 倍（中央値）**＝cap 5 目の手番で 2 目より高い手を
  一度も調べていない
- probe 済み候補の E と vloss は**正の相関 0.24**。E>=2 の候補の割合は vloss<0.3 で 6%、0.3〜1 で 4%、
  1〜2 で 10%、2〜4 で 11%、**4 目超で 21%**＝大きい罠は高い帯に多い
- 手番内の（最大 E − 最善手 E）は中央値 0.31・90%点 1.59＝探した範囲の中では差が小さい
- 互角帯（cap<=1.6）は 352 手番中 171＝半分。ここは代金の話なので今回は触らない

### 設計

`Enigma13PlusStrategy._shortlist(pool)`（基底は従来の `enigma9_shortlist(pool, 7)`＝ビット同一）を
`enigma9_shortlist_spread(pool, 7, probe_extra)` に差し替える: 従来の安い順 7 手＋残りの admissible 候補
（trusted＝visits>=10 を loss 昇順）から**等間隔に `probe_extra`（既定 4）手**（最安と最高を含む＝cap の
帯を端まで見る。trusted が足りなければ浅い候補を visits 多い順で埋める）。代金の上限（cap・勝率フロア・
cost_weight）は不変＝**同じ予算の中でより大きい罠を net 比較に乗せる**だけ。候補が 7 手以下の手番は
従来と同一。ΔE 床とは補完関係（rarity だけの外しに予算を食われないので、見つけた本物の罠に回る）。

**先読み wave2 の同期**（ユーザー要件「先読みで即着手できなくなるなら困る」）: 従来の wave2 は予測した
応手局面の visits 上位 8 手しか温めないので追加 4 手はコールドになる。`_ponder_wave2_targets(analysis,
player)` フック（基底は従来の上位 8 手＝ビット同一・既存の worker テストで固定）を難解＋が上書きし、予測
局面の root scoreLead から `enigma9_spending_plan` と同じ式で cap を近似 → 同じ spread 規則
（`enigma9_wave2_targets`）で温め対象を組む。勝率フロアは見ない（温めは上位集合でよい）。的中時の即着手は
保たれる（追加分が cache hit する）。

設定 `enigma13plus_probe_extra` 候補 0/2/4/6/8・既定 4（0 で従来の 8 手）。モジュール定数 `ENIGMA9_PROBE_EXTRA`。

### コスト

13路の子局面プローブは 8 手で 1.2〜1.4 秒（追記3）なので 12 手で +0.3〜0.6 秒/手と見込んだが、katrain_debug の
コールド実測は **+0.8 秒**（1.2 → 2.0 秒・下記リプレイ結果）。重ければ `probe_extra` 2 に落とす。監視モードで先読みが的中した手番は wave2 が 12 手ぶん温めているので従来どおり 0.3〜0.5 秒。

### 検証

- `tests/test_ai_enigma_plus.py`: `TestShortlistSpread`（6）・`TestWave2Targets`（5）・`TestShortlistHooks`（6）
  ＋設定/GUI/i18n 整合。既存 `TestPonderWorker::test_root_completion_issues_its_wave2_probes_and_the_next_root`
  （基底の wave2 対象 8 手）が不変を固定。enigma 関連 191 PASS
- katrain_debug リプレイ: 下記「リプレイ結果（probe_extra）」
- **未実施**: 実対局での realized 再計測。測り方は追記12 実測1（ログの lead 差分）。比較軸は
  1局あたりの「打った手の E 合計」と「相手の実損失合計」を難解（13路）と並べる

### 変わらないもの・留意点

- 選択式（E・reply_rare・own_rare・cost_weight）は据え置き。reply_rare は実測で寄与ゼロだが、探索範囲と
  選択式を同時に変えると効果の切り分けができないので今回は触らない（次の候補）
- 互角帯の cap は不変＝互角で大きい罠を買う改修ではない（代金の許容が「勝勢の余剰だけ」）
- 1手先の E しか測れない地平線は不変

### リプレイ結果（probe_extra・katrain_debug・復元 SGF `game_20260902_003240`・max_loss 1.6 / target 1.5）

- **`--move 68`（AI 黒 35手目・Spend cap 5.84・admissible 23 / trusted 8）**: `Probe spread: +4 from the wider
  loss range [J4 3.94, G7 4.68, M8 4.96, G6 4.47]`（trusted の残り1手＋浅い候補を visits 順に3手）。追加分の
  E は G7 1.71 / M8 0.27 / J4 0.19 / G6 0.20 で、最善 F6（E 3.00・net 3.95）を超える罠は無し。ΔE 床が 12→2
  （F6・K5）に絞り `Best move wins` → **F6**。着手決定 2.0 秒（コールド・追加4手込み。従来8手の同局面コールド実測は
  難解＋ 1.2 秒 / 難解 1.1 秒＝**+0.8 秒**。見込み +0.3〜0.6 秒を上回った＝追加分は NN キャッシュに無い
  局面なので GPU 時間がそのまま乗る。先読み的中時は wave2 が温めるので乗らない）
- **`--move 48`（AI 黒 25手目・cap 3.28・admissible 7）**: 候補が 7 手以下なので spread は発火せず（ログに
  `Probe spread` 行なし）。ΔE 床 8→4、L7（E 1.15・find 0.056・vloss 3.09）へ外し＝最善 H4 の E 0.29 に対し
  E を 0.86 買う本物の罠。着手決定 1.0 秒
- この局面では追加分から採用に至る罠は出なかった（最善手自身が E 3.0 の罠）。効果の有無は実対局の
  E 合計／相手の実損失で測る（追記12 実測1 の手順）

## 追記12-3（2026-09-02）: 難解＋の実対局結果（13路3局）と 9路・19路への展開

### 実対局（難解＋ 13路 3局 vs 難解 13路 残存5局・同じログ集計）

難解＋ `game_20260902_113242` / `123337` / `130322`（probe_extra 4・ΔE 床 0.2/0.3）。比較は追記12 実測1 の
7局のうちログが残っている5局（対局ログの保持は当時10本＝`base_katrain.KaTrainBase.LOG_KINDS`。**A/B の比較側が消えるので
2026-09-02 に 40 本へ引き上げた**（ユーザー指示）。

| | 難解（5局） | 難解＋（3局） |
|---|---|---|
| 外し率 | 68% | 66% |
| 打った手の E 平均/手 | 1.37 | **1.97** |
| E>=1 / E>=2 の手番 | 44% / 24% | **57% / 40%** |
| 払った vloss 平均/手 | 0.72 | 0.59 |
| 相手の実損失 平均/手（lead 差分・n=121 / 50） | 1.90 | **2.90** |
| 相手の失着 >=1目 / >=2目 | 49% / 28% | **60% / 56%** |
| 着手決定 中央値 / 平均 / 最大（秒） | 0.3 / 0.5 / 2.7 | 0.3 / 0.6 / 2.6 |

Probe spread は 137 手番中 37 で発火、追加候補が採用された手番 8（うち E>=1 は E12 1.45・L7 1.26 の2手。
残りは cheap 免除の安い手）。ΔE 床は 64 手番で計 201 候補を落とした。**局数が少なく相手も違う**ので方向の
確認にとどまるが、「罠の総量（E）が増え、相手の実損失が増え、払う代金は減り、着手時間は変わらない」と
設計の狙いどおりの向き。

### 9路の既存ログ（難解9・2局・AI 白）

admissible 中央値 1・8 手超は 48 手番中 10（プローブ済み最大 raw/cap 中央値 0.65＝13路の 0.35 より偏りは
小さい）。corr(E, vloss) 0.15、E>=1 の割合は vloss<0.3 で 28%・2目超で 55%＝高い帯に E が多い構造は同じ。
ΔE<0.2 かつ vloss>0.3 の外しは 18 手中 0＝**ΔE 床はこの2局では何も変えない**。

### 19路

監視対局のログが無く未計測。候補プールが最も大きい盤なので「安い順 7 手」の偏りは構造的に最も強い
はず（13路で pool>8 の手番の最大 raw/cap 0.35）。

### 判断と実装

ユーザー指示「9路・19路でも採用の価値があれば追加」→ 9路は spread に小さい余地（ΔE 床は無害）、19路は
構造上最も余地が大きい見込み、で**両方追加**。実装は Plus の3フックを `EnigmaPlusMixin` に集約し
`Enigma9PlusStrategy(EnigmaPlusMixin, Enigma9Strategy)` / `Enigma13PlusStrategy(…, Enigma13Strategy)` /
`Enigma19PlusStrategy(…, Enigma19Strategy)`。設定キー接頭辞 `enigma9plus_` / `enigma19plus_`、基底の10項目は
盤サイズごとの候補値・既定をそのまま（GUI 候補値の一致は `test_base_options_are_shared_with_the_base_strategy`
で固定）、`min_delta_e` 0.2 / `cheap_loss` 0.3 / `probe_extra` 4 は 13路の値を流用（**9路・19路は未校正**）。
ローカル config は基底の現在値を写した（9路: max_loss 1.6・large 6.0・min_wr 0.25・target 0.2・aim_jigo true /
19路: max_loss 2.6・endgame 180・locality 3.0）。i18n は 難解＋（9路）/（19路）・Enigma+ (9x9)/(19x19)。
CLI 名 `enigma9plus` / `enigma19plus`。テストは `tests/test_ai_enigma_plus.py` に3戦略をパラメトライズ
（228 PASS・enigma 一式込み）。

### 留意点

- 19路の probe は 1 本あたり重い（500v・19路）ので `probe_extra` 4 のコールド増分は 13路の +0.8 秒より
  大きい見込み。重ければ 2 に
- 9路は候補が少なく spread の発火自体が稀（10/48 手番）＝効果は限定的、害も無い
- 効果の測り方は追記12 実測1 と同じ（ログの lead 差分）。比較には基底の対局ログが要るので退避を

## 追記12-4（2026-09-02）: 9路の交互対局（難解 6局 vs 難解＋ 5局）＝利点を確認できず

追記12-3 の「9路は 2局 vs 2局で判定不能」を受け、ユーザーが 9路で難解／難解＋を**交互に**打った
（`game_20260902_155252` 〜 `164711` の 7局。`165216` は着手 0 で除外）。既存 4局と合わせ 難解 6局 / 難解＋ 5局。
集計は scratchpad `enigma_arm_summary.py`（`enigma_realized_probe.py` を局・腕別に束ねる。外し率＝AI 全手番中
`Deviate` の割合、実損失＝追記12 と同じ `lead(次の自手番) − lead(この手番) + vloss`＝勝勢・ヨセ手番のみ測れる）。

| 9路 | 局 | 外し率 | E/手 | vloss/手 | 実損失/手 pooled（n） | 中央値 | >=2目 | 着手中央 |
|---|---|---|---|---|---|---|---|---|
| 難解 | 6 | 35% | 1.74 | 0.60 | 4.27（32） | 3.28 | 62% | 0.15 s |
| 難解＋ | 5 | 33% | 1.84 | 0.36 | 1.65（46） | 1.37 | 30% | 0.10 s |

- **E は +0.10（z 0.3）＝差なし**。外し率も 35%→33% で不変。
- 実損失は難解側が高いが、比較として成立していない: 9路で測れる手番は 1局 3〜13 しかなく、`164711` は
  lead 40→73→80→87 の崩壊局（実損失 18.8 / 32.4 の 2 手番）が難解側の平均を支配する。外れ値を除いても
  median 3.28 vs 1.37 で難解側が高く、**難解＋が 9路で罠の効きを上げている証拠は無い**。
- 機構の発火が小さい: spread は 3〜7 手番/局（13路は 8〜25）、ΔE 床は 8〜46 候補を落とすが外し率が
  動かない＝落ちているのは元々選ばれない候補。効いているのは vloss 0.60→0.36（安く済む側）だけ。
- 13路は新規ログなし（難解 5 vs 難解＋ 7 のまま。本スクリプトの値: E 1.35→1.64〈z 0.8〉・実損失 pooled
  1.90→2.82〈z 1.4〉・>=2目 28%→48%・vloss 0.70→0.61・着手中央 0.30→0.40 s）。

**結論**: 9路の難解＋は「利点なし・害も確認できず」で校正を打ち止める（9路は spend 手番が少なく、局数を
足しても分解能が出ない）。9路の既定は難解（基底）のままで良い。難解＋の効果検証は 13路に集中する
（2σ には片側 12 局程度＝難解側をあと 7 局、交互に）。19路は引き続きログなし・未校正。

## 追記12-5（2026-09-03）: 13路の交互対局を追加＝難解 13局 vs 難解＋ 14局で相手の実損失 +1.0 目/手（約 2σ）

追記12-3 の 5 局 vs 7 局（z 0.9〜1.4）に、ユーザーが難解／難解＋を**交互に**打った 15 局
（`game_20260902_175413` 〜 `game_20260903_005312`。難解 8 局・難解＋ 7 局）を足した。集計は追記12-4 と同じ
`enigma_arm_summary.py`（実損失＝`lead(次の自手番) − lead(この手番) + vloss`＝勝勢・ヨセ手番のみ）。

| 13路 | 局 | 外し率 | E/手 | vloss/手 | 実損失/手 局平均（局sd） | pooled/手（n） | 中央値 | >=1目 | >=2目 | 着手中央 |
|---|---|---|---|---|---|---|---|---|---|---|
| 難解 | 13 | 56% | 1.66 | 0.93 | 2.27（1.20） | 2.25（276） | 1.21 | 57% | 41% | 0.35 s |
| 難解＋ | 14 | 51% | 1.84 | 0.85 | 3.34（1.54） | 3.23（205） | 2.28 | 67% | 54% | 0.40 s |
| 差 | | −5pt | +0.18（z 0.7） | −0.08 | **+1.07（z 1.9）** | **+0.98（z 2.4）** | | | | |

**相手のレベル変動を切るため、交互に打った 15 局だけでも比較した**（同じ時間帯・同じ相手・vloss/手は両腕 1.08 で同一）:

| 13路・交互 15 局のみ | 局 | E/手 | vloss/手 | 実損失/手 局平均 | pooled/手（n） | 中央値 | >=2目 |
|---|---|---|---|---|---|---|---|
| 難解 | 8 | 1.85 | 1.08 | 2.45 | 2.52（155） | 2.02 | 50% |
| 難解＋ | 7 | 2.03 | 1.08 | 3.86 | 3.71（95） | 2.57 | 61% |
| 差 | | +0.18（z 0.5） | 0.00 | **+1.41（z 2.7）** | **+1.19（z 2.6）** | | |

最初の 12 局（難解 5 / 難解＋ 7・非交互）だけの差は +0.92（z 0.9〜1.4）で、**両バッチが同じ向き・同じ桁**。

読み方:
- **相手の実損失は 1 手あたり約 +1.0 目、局平均で z 1.9・手 pooled で z 2.4、交互バッチだけなら z 2.6〜2.7**
  ＝ユーザーの意図（相手がより大きい損失手を打つ）が実測で確認できた。同じ vloss（払う代金）で回収が
  2.3〜2.4 倍 → 3.4〜3.8 倍（pooled 実損失 / vloss）。
- **E/手の差は +0.18（z 0.5〜0.7）で有意でない**。E は較正済みの予測値（追記12）なので、E がほぼ同じで
  実損失だけ増えるのは (a) ΔE 床が「最善手より E の低い手を珍しさで買う外し」（realized −0.5 目）を止めて
  最善手に戻すぶん E 平均を下げつつ実損失を上げる、(b) spread が高い帯の罠を拾う、の混合で説明できるが、
  内訳の分解はしていない（`Probe spread:` 発火 7〜27 手番/局・ΔE 落とし 25〜122 候補/局）。
- 難解側の E/手 が 1.35（最初の 5 局）→ 1.85（交互 8 局）と上がっている＝相手の打ち方が変わった証拠で、
  非交互の最初のバッチだけで結論しなかったのは正しかった。
- 着手時間は中央 0.35→0.40 s、最大 1.9→2.6 s（コールド手番の probe_extra 4 本ぶん）。

**結論**: 13路の難解＋は「同じ代金で相手の実損失 +1 目/手」を約 2σ で確認。既定値（`min_delta_e` 0.2 /
`cheap_loss` 0.3 / `probe_extra` 4）はこのまま。9路は追記12-4 のとおり利点なし、19路は引き続き未校正。
