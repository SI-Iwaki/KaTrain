...
**9路**: 相手の一致率の実測が無いので、先に保持中の難解＋9路ログ17局を SGF に戻して事後解析し、目標を作ってから同じ手順で校正する。手順と結果は韜晦の spec（`2026-09-23-veil-strategy-design.md`）§16（2026-09-26）: 復元は `recon_logs.py --size 9`（持碁9路の1局を除く難解＋の局＝`recon_9/`・局数は韜晦の spec §16.2 手順1。一致率ひかえめ9路の実戦は `--strategy Veil9Strategy` で `recon_9_veil/` に分ける）・区間は 12 / 41 手・投了の手数は `recon_9/` を縮めずに使う（lead モデルは 9路では `--resign-lead` が要る）・校正の AI は実戦の多数派の設定（`calibrate --strategy enigma9plus:<上書き>`）・目標は `CALIB_TARGETS_9`。
