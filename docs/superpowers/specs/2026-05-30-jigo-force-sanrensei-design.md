...
`_get_star_lines` の各辺（3点コリニア）について AI石数・相手石数・空点を集計し、**AI が最も石を置いた「コミット済みライン」**を基準に判定する。

- AI 星石 0 個（初手）→ 相手石が無いラインの空き隅星（中辺星は最初に出さない。三連星はどの隅からでも開始可能）
- AI 石数の最大値を `max_ai` とし、`ai_count == max_ai` のラインを「コミット済みライン」とする:
  - コミット済みラインのうち**相手石が乗っていないもの**があれば、その空き星点を target とする（完成を目指して続行）
  - AI 石 1 個（隅星）の段階では、その隅を通る2ラインが候補になり humanPolicy で伸長方向が決まる。次手で同辺2点に揃うと `max_ai=2` となりラインが一意化する
  - AI 石 2 個（同辺）→ 残り1点（中辺星 or 残隅）で三連星完成
- **コミット済みラインがすべて相手に妨害された（相手石が乗った）→ 空集合（三連星は崩れたとみなし強制終了し、通常 jigo に戻る）**
- AI 石 3 個が一直線に揃ったライン → 完成、空集合（停止）

> **妨害時の挙動（重要）**: 黒が2子置いたラインの中辺星等を白が取って妨害した瞬間、別ラインへ pivot せず**即座に星打ち強制を打ち切り通常 jigo に戻る**。「妨害されても星を打ち続ける」粘着挙動は人間らしくないため採用しない。なお AI 石1個の段階では、片方のラインが妨害されてももう片方の有効ラインで続行する（まだ三連星に「コミット」していないため）。
