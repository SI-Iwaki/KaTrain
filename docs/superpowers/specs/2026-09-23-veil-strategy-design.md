...
- 追記（2026-09-26）: 同じユーザーのローカル 13路設定（`new`）は、2026-09-26 の接戦ストレス 13-4（同じ seed 1000〜1019）では 18-2-0 に落ちた。run をまたぐと結果が変わる（40-0-0 → 18-2-0）のは KataGo の探索の非決定性が原因と見られる（合わせて 60 局で 58-2-0）。詳細は §16.6・`veil13-campaign.md` の「13-4 接戦ストレス」。
...
「序盤の研究外し（spec §15・§16.4）の計測（2026-09-26）」）。コードは calib9 は HEAD `e39e3e93`・スモーク以降の段階の run は `0a6f0bfe`（どちらも dirty false）・設定の写しは全実行で同じ・aborted 0。
