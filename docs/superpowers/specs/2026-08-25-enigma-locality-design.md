...
距離分布（`>=4`）は off→σ3・off→σ4 のどちらも一貫して減少し run 間ノイズ（`ai_top_move` ±0.03・
`mean_ptloss` ±0.05）を超える差。損失側は σ3 が off 同等〜やや良好（`mean_ptloss` off比 −0.029・
...
かった＝損失増の原因はタイブレーク機構よりも KataGo 解析のプロセス間非決定性（候補の net 自体が
run ごとに変わる）である可能性が高い。**19路は既定 OFF のまま・σ5 は方向確認の域を出ず未校正・要3run
再測**（本タスクのスコープ外）。
