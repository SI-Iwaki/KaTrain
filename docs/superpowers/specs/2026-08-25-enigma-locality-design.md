...
帯は net 単位＝選択基準と同スケール。勝勢時の消費モードでは loss 項が cost_weight で
割引されているので、0.3 net ≒ 0.3/cost_weight 目の生損失差まで帯に入る（cap で有界）。
raw 目単位（slack×cost_weight）の帯にする案は 19路 3run 再測時の検討項目。
...
実測データ・生ログは `docs/superpowers/specs/calibration-data/enigma9/enigma-locality-results-20260825.md`。

### 13路（白50手×3run平均）

| arm | 外し率 | total_loss | dist(選択手,直前の相手手)>=4 | dist(選択手,KataGo最善手)>=4 | ai_top_move | mean_ptloss |
|---|---:|---:|---:|---:|---:|---:|
| off | 72.0% | 86.93 | 18.67 | 25.33 | 0.280 | 1.739 |
| σ3  | 66.7% | 85.51 | 14.00 | 18.00 | 0.333 | 1.710 |
| σ4  | 69.3% | 88.99 | 13.67 | 20.00 | 0.307 | 1.780 |

距離分布（`>=4`）は off→σ3・off→σ4 のどちらも減少方向（対 ai_top は off 24–27 vs σ3 15–22 vs σ4
16–22 で run ごとの範囲が完全に分離、対 opp_last は off 16–21 vs σ3 12–16・σ4 13–14 で σ3 run3=16 と
off run3=16 がタイ）。`ai_top_move` ±0.03・`mean_ptloss` ±0.05 の run間ノイズ閾値は distance には
適用できない（distance の run間ノイズ幅は未推定）。損失側は σ3 が off 同等〜やや良好（`mean_ptloss` off比 −0.029・
`total_loss` off比 −1.42）なのに対し σ4 は σ3 比 `mean_ptloss` +0.070（実差）で悪化方向。distance
縮小幅も σ3（対最善手 −29%）が σ4（−21%）より大きい。

point_loss 5.9〜7.4・チェビシェフ距離 5〜10 の「本物の遠い罠」3手番を個別に σ3 ON で再実行したところ、
3手番とも `Band: 1`（同点帯に対抗馬なし）でそのまま単独選択され続けた＝σ3 はこの種の遠い罠を握りつぶさない。

**推奨**: 13路は σ=3.0。distance 縮小効果が σ4 より大きく、損失側は off 同等以上、遠い罠も保持される。
既定値は変えない（OFF のまま）＝rules 表には推奨値として記載。

帯は net 単位＝選択基準と同スケール。raw 目単位（slack×cost_weight）は再測時の検討項目。

prox でスケールされた own_rare は挑戦者側の net だけを下げ、最善手（prox=1.0 のアンカー）の
net は不変。この非対称さにより ON は選択手が最善手 net+margin を下回って外しを止める側にも、
元は margin を割っていた挑戦者が浮上して外す側にも倒れうる。外し率 72.0%→66.7%・`ai_top_move`
+0.053 はこの機構の結果。

### 19路（`tests/data/ogs.sgf` 白56手・**1run のみ**）

| arm | dist(opp_last)>=4 | mean_ptloss | total_loss |
|---|---:|---:|---:|
| off | 24 | 0.312 | 17.45 |
| σ5  | 17 | 0.519 | 29.08 |

対直前手の `>=4` は減少方向（局所性の効果と整合）だが、`mean_ptloss`/`total_loss` は run間ノイズ閾値を
大きく超えて悪化。損失増は終盤の3手番（move_num 94/100/108 付近、point_loss 3.36/3.65/4.45）に集中して
いたため、`--move 93/99/107` で OFF/ON を個別に再実行して機構を確認した（別プロセスなので `--batch` の
値とは一致しない＝KataGo の run 間非決定性）。3手番とも ON 側の `Band:` は **1**（同点帯に対抗馬なし＝
locality のタイブレークそのものは発火していない）で、move 93/107 は OFF/ON でほぼ同じ手・move 99 は
むしろ ON の方が vloss が小さい手を選んでいた。想定していた「消費モード（`Spend`）の cost_weight 割引に
よって同点帯内で許容される生損失が増える」というメカニズムは、この3手番では Band=1 のため観測されな
かった。**原因は未特定**（§4.2 の own_rare 減衰は Band=1 でも net を変えるので、Band=1 は局所性が効か
なかった証拠にならない。単発 run の +11.6 目は想定ノイズの約4倍で、OFF の2本目も未取得）。3run 再測
（OFF は最低2本）まで結論しない：**19路は既定 OFF のまま・σ5 は方向確認の域を出ず未校正**（本タスクの
スコープ外）。

### §3 保留項目（第3アンカー＝自分の前手）の要否

13路の実測データでは (a) 相手の直前手アンカー＋(b) KataGo 最善手アンカーの2アンカーだけで
`dist(選択手, opp_last)>=4` が 18.67→14.00 と縮小しており、第3アンカーが無いと解決できない
残留パターン（「打ち込みの次の手が別の場所」）は今回のデータからは確認されなかった。**保留を継続**
（19路データが揃ってから再評価する）。
