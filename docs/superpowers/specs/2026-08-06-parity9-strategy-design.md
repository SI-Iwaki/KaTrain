...
**未校正**: `parity9_unsettled_max` の既定8と `PARITY9_UNSETTLED_ABS` の 0.5 は根拠のない当て推量。実戦ログで「実際にヨセに入った手数」と未確定点数の対応を1〜2局取ってから決める。`parity9_endgame_move` の既定30は既存の 9路ヨセ閾値（力戦派 32、`jigo9_endgame_move` 既定30）に合わせた値。
