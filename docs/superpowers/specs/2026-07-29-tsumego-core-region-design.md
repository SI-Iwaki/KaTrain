...
コアの選び方はエスカレーション梯子。「gap を小さくすると最大クラスタは縮む → bbox が縮む →
外側面積は増える」ので面積テストは gap に対して単調。したがって
「cluster_gap から降順に見て最初に通る gap」と「通る中で最大の gap」は同値であり、
**辺を1回だけ作って gap 昇順に増分 union する**ことで O(n^2) 1パスで求まる:

```
石対の Chebyshev 距離を1回だけ走査し、距離 d (1..cluster_gap) ごとの辺リストを作る
for gap in 1..cluster_gap:
    距離 gap の辺を union して最大クラスタを記録
通った（外側面積 >= needed を満たす）中で最大の gap のクラスタを採用
どの gap でも満たせなければ gap=1 の最大クラスタを採用（best effort）
```

gap ごとに union-find をやり直す素朴実装は 4 倍のコストになる（下記「性能」参照）ため採らない。
...
```
tsumego case: 13x13, 30 stones
  current main_cluster (1 pass)       0.17 ms
  naive ladder (4 passes)             0.57 ms
  incremental ladder (1 pass)         0.19 ms

ogs.sgf move 113: 19x19, 102 stones     ← 最悪ケース
  current main_cluster (1 pass)       1.47 ms
  naive ladder (4 passes)             5.21 ms
  incremental ladder (1 pass)         1.46 ms
```

増分 union 方式なら現行と同コスト（最悪 1.5ms）。素朴な4段梯子は 4 倍かかるため採用しない。

画面キャプチャ・盤面認識は変更なし。局面構築は微減する（現行は `_do_new_game` の後に
枠ノードを追加して chains を再構築するが、再構築方式は new-game 1 回で済む）。

### KataGo 解析側 — 未測定、1秒前後の増加見込み

| | 現行（枠が退化している状態） | 修正後 |
|---|---|---|
| クエリ | 全盤 1500 visits・8秒上限 | 全盤fast 300 visits + リージョン限定 1800 visits・時間上限なし |
| 合計 visits | 1500 | 2100 |

visits 増加分だけ遅くなる見込み。ただしこの2段解析は新設ではなく、
**枠が正しく張れているキャプチャでは既に走っている経路**であり、
修正後の速度は現在正しく動作しているキャプチャと同じになる。遅くなるのは現在誤答しているケースのみ。

速度を優先する場合は `tsumego_capture.analysis_visits`（現在 1800）で調整できる。
実装時に実測して、体感で問題がある場合は既定値の見直しを検討する。
