...
設計と実測の一次記録。全57本。**サイズが大きいものは Read で全読みせず、Grep で該当節だけ引くこと**。

凡例: 🟢 実装済み ／ 📝 設計済み・実装待ち ／ ⛔ 棚上げ・未実装（コードに存在しない） ／ 📄 調査・解析のみ（実装を伴わない）
...
| `2026-09-23-veil-strategy-design.md` | 🟢 韜晦（9/13/19路・画面の名前は 2026-09-25 から「一致率ひかえめ」）`ai:veil9` / `veil13` / `veil19`＝勝ち優先で自分の一致率を絶対目標（13路 30%）まで下げる。同値外しは常に・支払う外しと明らかな一手の外しは一致率が目標超のときだけリード余剰から・外しは子局面で検証し（決着局面の即決は best と候補の2手だけプローブ〈2026-09-25〉・終局帯の手は通常の解析で決める）最安帯の hp 最大・ヨセは委譲しない・ponder なし・罠は A/B 用スイッチ・失着の層（§13・2026-09-25・既定 OFF・LOG／ON。計画 `plans/2026-09-24-veil-blunder.md`）。ハーネス（`calibration-data/selfplay/veil13-campaign.md`）の実測は自分の一致率 spec の初期値 3 run 平均 51.5%・loose 45.2%＝目標に届かず・下限は構造的（段階1で最善手を打つしかない手番 52.7%）・**13路の既定は 2026-09-25 のユーザーの決定で段階1b の loose**（安全条件は据え置き）・接戦ストレス（AI 不利 4 目・HumanStyle 9段・20 対）で勝ちの安全は初期値・loose とも可（どちらも 20-0・難解＋は 17-2-1 / 19-1。攻めプリセットは未測定）・9/19路は未校正・**実戦校正は未実施**・§14 最善手しか無い手番の外し（2026-09-25・既定 OFF）・§15 序盤の研究外し（2026-09-26・📝 設計済み・実装待ち・既定 OFF・窓の中は難解＋に任せる）・§16 9路の校正と研究外しの計測（📝 計画） |
...
| spec | |
|---|---|
| `2026-08-18-board-watch-design.md` | 🟢 **87KB**・`board_watch.py`。対局アプリの着手を検出して人間側の手として片方向注入（トグル `ctrl+alt+d`）。追記10 で連続対局（対局者アイコンから KaTrain の AI の色を自動判定・次の対局で監視を自動で張り直す） |
...
54本。上の各 spec と**1対1で対応する実装チェックリスト**（ファイル名は spec から `-design` を
...
`2026-09-24-veil-blunder.md` は独立の spec を持たない追加の計画（韜晦の spec `2026-09-23-veil-strategy-design.md` の §13 失着オプション。13路の既定を loose にする変更と決着局面の即決の検証も含む）。
