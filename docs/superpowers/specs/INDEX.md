...
| `2026-09-23-veil-strategy-design.md` | 📝 韜晦（9/13/19路）`ai:veil9` / `veil13` / `veil19`＝勝ち優先で自分の一致率を絶対目標（13路 30%）まで下げる。同値外しは常に・支払う外しと明らかな一手の外しは一致率が目標超のときだけリード余剰から・全外しを子局面で検証し最安帯の hp 最大・ヨセは委譲しない・ponder なし・罠は A/B 用スイッチ。反実仮想の見込みは 13路 平均約 46%（目標に届く局は少ない）＝ハーネスで境界線を測って既定値を決める |
...
| `2026-09-23-selfplay-harness-design.md` | 🟢 自己対局ハーネス `katrain_debug.selfplay`＝戦略 vs humanSL ボット（実戦の相手に校正）を無人で N 局打たせ、本物の `game_report` で両者の一致率・勝敗・目差を集計。seed 対の ABBA・接戦ストレス層・投了なし・影判定・hp 監査。実装 `katrain_debug/selfplay*.py`・データ `calibration-data/selfplay/`（相手ボットは校正済み: rank_1k / rank_1d / rank_3d・τ 1.0・結果 `selfplay/selfplay-calibration-results-20260924.md`） |
