...
設計と実測の一次記録。全54本。**サイズが大きいものは Read で全読みせず、Grep で該当節だけ引くこと**。
...
| `2026-09-17-mimic13-strategy-design.md` | ⛔ **未実装**（設計のみ・2026-09-17）擬態（13路）`ai:mimic13`＝相手より低い一致率で勝つ。price = vloss − ΔE をリード連動の λ で買う・自然さ（humanPolicy）か罠（ΔE）の資格・ヨセは 9段委譲 |
