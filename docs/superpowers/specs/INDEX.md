...
| `2026-09-23-veil-strategy-design.md` | 🟢 韜晦（9/13/19路）`ai:veil9` / `veil13` / `veil19`＝勝ち優先で自分の一致率を絶対目標（13路 30%）まで下げる。同値外しは常に・支払う外しと明らかな一手の外しは一致率が目標超のときだけリード余剰から・外しは子局面で検証し（決着局面の即決と終局帯の手は通常の解析で決める）最安帯の hp 最大・ヨセは委譲しない・ponder なし・罠は A/B 用スイッチ。ハーネス段階1（2026-09-24・20局 × 5アーム）の実測は既定で自分の一致率 局平均 53.1%（相手 26.3%・20/20 勝ち）＝目標に届かず・下限は構造的（最善手を打つしかない手番 52.7%）で既定値は据え置いた（`calibration-data/selfplay/veil13-campaign.md`）・段階3（接戦ストレス＝AI 不利 4 目・HumanStyle 9段・20 対）で勝ちの安全は既定の設定で可（20-0・flip 0.20/局 vs 難解＋ 2.35/局・難解＋は 2 敗。攻めプリセットは未測定）・段階1b/2 は未実施・**実戦校正は未実施** |
