...
Expected: 2173 件すべて PASS（Plan A の終わりの 2120 件〈Plan A の計画の Task 7 Step 1 の期待値〉＋この計画の 53 件＝Task 1 2・Task 2 3・Task 3 3・
...
```
Expected: Plan A と Plan B の Task 1〜10 のコミット・`{9: 'ai:enigma9plus', 13: 'ai:enigma13plus', 19: 'ai:enigma19plus'} 0`・2173 件すべて PASS
（Task 10 Step 2 と同じ）。
...
（データの注意: `recon_9/summary.json` の `settings` は局の最初の `Initializing` 行のまま＝`214631`・`222543` は途中で `overdraft_answered_max` が 99→0 に変わり、`100100` は `aim_jigo`・`gamble_until_move` が変わった〈Task 7 Step 4 の `settings changed during the game:` 3 行〉。C4 Step 1 の多数決の上書きはこの最初の設定のまま計算する＝この3局はデータ上の注意点。C9 の準備の節にも同じ注意を書く。）
...
- `grep -c '"decision_open": "played"' $RUN/moves.jsonl` が 1 以上・`grep -h "VeilOpenEnigma9PlusStrategy" $RUN/logs/open_*.log | head -3` に行がある・
  ...
  かつ `games_with_open=N/N`（分子=分母＝open アームの全局で研究外しが発動。finding 2）・`open` の played+gate+invariant の合計が
  窓の中の open アームの AI の手番の総数と一致する
...
- [ ] **Step 1: `docs/superpowers/specs/calibration-data/selfplay/veil9-campaign.md` を作る**（Write。CRLF にする）。節と中身:
  ...
  - `## 準備`: recon_9（16 局・直したこと）・事後解析・`CALIB_TARGETS_9`（calib_targets の出力の最後の3行）・校正（段位ごとの表と選んだプール・
    ずれ・手数の中央値 vs 48）。
  - **データの注意**（C2 で確かめた点）: recon summary の `settings` は局の最初の `Initializing` 行のまま（`214631`・`222543` は途中で `overdraft_answered_max` が 99→0 に変わり、`100100` は `aim_jigo`・`gamble_until_move` が変わった。C4 の上書きはこの最初の設定で計算する）。また、自分・相手の一致率（top1）と `window_stats.py --recon` の `opp_streak` は完全一致でしか数えないため、盤がほぼ空の序盤（1〜3 手目・§16.1 の 4-4/4-5/天元/3-5 のような 0.12目以内の同格の初手）では、対称で同格な手を打っても不一致に数える（過小評価の方向。C10 の実戦の比較にも同じ注意）。
...
- [ ] **Step 1: 結果を見せて決めてもらう**（spec §16.5。コードもユーザー設定もここでは変えない）:
  ...
  4. 実戦の確認（spec §16.5 の「実戦」）: 研究外しを ON にした実戦は各 10 局以上・実戦の直後にログを `experiments/selfplay/realgames-9x9-logs/` に退避する。
     ...
     で難解＋9路の局（`recon_9/`）と比べる（相手の最善手との一致率・序盤で相手が最善手を続けた手数〈`streak`〉・窓の後の相手の一致率〈`opp out`〉。序盤〈1〜3 手目〉の対称で同格な初手〈§16.1 の 4-4/4-5/天元/3-5〉は完全一致でしか数えないため streak・一致率とも過小評価になりうる〈C9 の準備の節と同じ注意〉）:
