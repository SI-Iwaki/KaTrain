...
| 系統 | 内容 | 触る前に読む |
|---|---|---|
| Human-like AI 戦略 | 悪手フィルタに加え、力戦派 / 攻城 / 狩猟 / 一致率低減 / 持碁 / 一致率追随（9路）/ 難解（9・13・19路）/ 難解＋（9・13・19路＝罠探索の拡張＋ΔE 床の改良版・`EnigmaPlusMixin`）/ 擬態（13路＝相手より低い一致率で勝つ・`Mimic13Strategy`）/ 一致率ひかえめ（旧名 韜晦・9・13・19路＝勝ち優先で自分の一致率を絶対目標まで下げる・序盤の窓を難解＋に任せる研究外しの層〈既定 OFF〉つき・`Veil9Strategy`） | `.claude/rules/ai-strategies.md`（設計と実測）<br>`.claude/rules/ai-parameters.md`（全パラメータ値）<br>`.claude/rules/ai-humanstyle.md`（フィルタ実装） |
...
- **winrate は常に黒視点**: `engine.py:127` で `reportAnalysisWinratesAs = "BLACK"` ハードコード。打つ側視点にするには `wr if player=="B" else (1-wr)` で変換
...
katrain/
  core/               -- コアロジック（主要ファイルのみ記載）
    ...
    screen_marker.py  -- 対局監視で AI の着手をアプリ盤の真上に輪で示す（Win32 の最前面・クリック透過窓。Kivy 非依存）
    screen_cursor.py  -- 対局監視で AI の着手の交点へマウスカーソルを運ぶ（SetCursorPos のみ・クリックしない。Kivy 非依存）
...
- **詰碁ログの横断検索に Grep ツールを使わない** — 30日より古い問題は `logs/archive/*.zip` の中にいてGrep からは**見えない**（ヒット0が「そのログは無い」と読めてしまう静かな失敗）。`python tools/grep_tsumego_logs.py "<正規表現>"` を使う＝平文と zip を横断する。個別ログの読解は従来どおり Grep でよい
...
- **Kivy/KivyMD のウィジェットを「状態を合わせるため」に作り直さない** — KivyMD のボタン類は `__init__` でグローバルな `theme_cls` に bound method を bind し、`theme_cls` のプロパティは一度も変わらないので死んだ WeakMethod が掃除されず溜まる。Kivy の `bind()` は重複チェックで既存 observer を**全部デリファレンス**するので 1 回の bind が observer 数に比例して遅くなる（実測 0.012ms@0 → 6.3ms@20k）。`CollapsablePanel` がプレイ／解析のモード切替のたびにタブボタンを作り直していたため、詰碁の自動ループでキャプチャ反映が **+12〜16ms/問 ずつ線形に**伸びた（300 問で 1.7→5.7 秒。オブジェクト数・メモリ・スレッド数は平坦＝普通のリーク指標では見えない。実測 2026-08-23・spec `2026-08-23-tsumego-capture-latency-growth.md`）。GUI スレッドの停止は関数をラップしても見えないので、フレーム間隔＋停止中の `sys._current_frames()` サンプリング＋`gc.get_objects()` の型別センサス差分で探す（プローブ `docs/superpowers/specs/calibration-data/tsumego/capture_loop_latency_probe.py`）
