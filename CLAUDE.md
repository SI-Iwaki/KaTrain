...
**ログファイル**（`C:\Users\iwaki\.katrain\logs\`）: 詰碁は**1問1ファイル**（`tsumego_<日付>_<時刻>.log`）。キャプチャの先頭で開くので、認識盤面（黒/白の GTP 座標）・抽出・出題前の検算・枠の採否・各手の判定が1ファイルに揃う＝**不具合報告はこのファイルを見れば再現できる**（ターミナルからコピーする必要はない）。`debug_level` 0 でも作られる（0 は INFO 行のみ、1 でエンジンのクエリまで）。対局は従来どおり `game_<日付>_<時刻>.log`（20手未満は無効試合として削除）。保持数は種別ごとに独立（詰碁30本・対局10本＝`base_katrain.KaTrainBase.LOG_KINDS`）で古い順に自動削除。spec 追記11
