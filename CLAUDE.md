...
- **大差フィルター（9路盤のみ）**: `analysis["rootInfo"]["winrate"]` を使用。`rootInfo.winrate` は常にBlack視点のため、White番は `1.0 - winrate` で変換。
  - **大差勝ち（勝率95%+）**: 最善手（`best_gtp_by_score`）を除外してhumanPolicy重みで選択。推奨手が最善手のみの場合は最善手を打つ。`WIN_RATE_THRESHOLD = 0.95`
