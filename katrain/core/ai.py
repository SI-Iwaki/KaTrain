...
class JigoStrategy(AIStrategy):
    ...
    def generate_move(self) -> Tuple[Move, str]:
        ...
        if not default_analysis or not default_analysis.get("root"):
            ...
            move_infos = [{"move": c["move"], "scoreLead": c.get("scoreLead", 0)} for c in self.cn.candidate_moves]
            if not move_infos:
                self.game.katrain.log("[JigoStrategy] No moveInfos, passing", OUTPUT_DEBUG)
                return Move(None, player=self.cn.next_player), "No moveInfos, passing"
    ...
    def _generate_human(self) -> Tuple[Move, str]:
        ...
        if move_infos:
            ...
            good_moves = _filter_moves(
                move_infos, BAD_MOVE_THRESHOLD, chaos_relax, ownership_grid, opponent_coords, player_sign, best_score
            )
            # --- 段階的閾値緩和フェイルセーフ ---
        ...
        ai_thoughts = f"\n{top_str}\n\nHuman+Fighting: played {move.gtp()} " f"({filtered_count} bad moves filtered)"
        return move, ai_thoughts
