...
def evaluate_pursuit_targets(
    ...
    for prev_target in previous_targets:
        ...
        min_dist = min(max(abs(ox - cx), abs(oy - cy)) for cx, cy in prev_coords)  # Chebyshev distance
...
def _jigo_relax_filters(candidates, max_loss, min_hp, hard_floor=MIN_HP_HARD_FLOOR):
    ...
    for reason, hp_factor in reason_map:
        ...
        f = [c for c in candidates if c["loss"] <= max_loss and c["hp"] >= threshold]
        if f:
            return f, reason
    threshold = max(min_hp * 0.25, hard_floor)
    f = [c for c in candidates if c["loss"] <= max_loss * 1.5 and c["hp"] >= threshold]
    if f:
        return f, "loss_150"
...
def _select_rank_by_lead(current_lead, target_score_max, base_profile, delta_1=5, delta_2=15):
...
def _jigo_resolve_phase(board_size, move_num, current_lead, phase_table_override=None, target_overrides=None):
    ...
    table = (
        ...
        if phase_table_override is not None
        else JIGO_DECEPTION_PHASE_TABLE.get(board_size, JIGO_DECEPTION_PHASE_TABLE[19])
    )
    base_phase = "phase0"
...
class JigoStrategy(AIStrategy):
    ...
    def generate_move(self) -> Tuple[Move, str]:
        ...
        engine.request_analysis(
            self.cn,
            callback=_set_stage1,
            error_callback=_err_stage1,
            priority=PRIORITY_EXTRA_AI_QUERY,
            include_policy=True,
        ...
        if stage1_error or not stage1_analysis or "humanPolicy" not in stage1_analysis:
            self.game.katrain.log("[JigoStrategy] Stage1 failed, falling back to KataGo top move", OUTPUT_DEBUG)
            candidate_moves = self.cn.candidate_moves
            if not candidate_moves:
        ...
        if (
            self.settings.get("jigo_force_sanrensei", False)
            and self.game.board_size[0] == 19
            and self.game.board_size[1] == 19
        ):
            ...
            if target_stars:
                ...
                self.game.katrain.log(
                    f"[JigoStrategy] force_sanrensei: n={n_star}, " f"targets={sorted(target_stars)}, chose={coords}",
                    OUTPUT_DEBUG,
                )
        ...
        engine.request_analysis(
            self.cn,
            callback=_set_stage2,
            error_callback=_err_stage2,
            priority=PRIORITY_EXTRA_AI_QUERY,
        ...
        if stage2_error or not stage2_analysis:
            ...
            self.game.katrain.log("[JigoStrategy] Stage2 failed, using Stage1 moveInfos (biased)", OUTPUT_DEBUG)
        ...
        self.game.katrain.log(
            f"[JigoStrategy] Stage2 query complete ({len(candidates)} candidates, " f"best_score={best_score:.2f})",
            OUTPUT_DEBUG,
        )
        ...
        self.game.katrain.log(
            ...
            f"(loss<={effective_max_loss}, hp>={min_hp})",
            OUTPUT_DEBUG,
        )
        ...
        if not filtered:
            ...
            self.game.katrain.log(
                f"[JigoStrategy] Fallback triggered: reason={reason}, {len(filtered)} candidates", OUTPUT_DEBUG
            )
            if reason == "safety_valve":
                self.game.katrain.log("[JigoStrategy] Safety valve: using KataGo top move", OUTPUT_ERROR)
...
class FightingStrategy(PickBasedStrategy):
    ...
    def generate_move(self) -> Tuple[Move, str]:
        ...
        if (
            self.settings.get("force_tengen_opening", False)
            and self.cn.next_player == "B"
            and len(self.game.stones) == 0
        ):
    ...
    def _build_complexity_weight_dict(self):
        ...
        return _apply_cut_boost(base_weights, self.game.board, self.game.chains, opponent_player, cut_boost)
    ...
    def _generate_scoreloss(self) -> Tuple[Move, str]:
        ...
        good_moves = [
            d
            for d in candidate_moves
            if d["pointsLost"] < fighting_max_loss and not Move.from_gtp(d["move"], player=self.cn.next_player).is_pass
        ]
        self.game.katrain.log(
    ...
    def _generate_human(self, complex_mode: bool = False) -> Tuple[Move, str]:
        ...
        self.game.katrain.log(
            f"[FightingStrategy:human] Stage 1: requesting humanSL analysis ({human_profile})", OUTPUT_DEBUG
        )
        ...
        if error or not analysis or "humanPolicy" not in analysis:
            self.game.katrain.log(
                f"[FightingStrategy:human] Stage 1 failed, falling back to scoreloss mode", OUTPUT_DEBUG
            )
            return self._generate_scoreloss()
        ...
        if clean_analysis and not clean_error:
            ...
            self.game.katrain.log(
                f"[FightingStrategy:human] Using clean moveInfos ({len(move_infos)} moves)", OUTPUT_DEBUG
            )
        else:
        ...
        is_area_scoring = (
            isinstance(_rules, str)
            and _rules.lower() in ["chinese", "aga", "tromp-taylor", "new zealand", "stone_scoring"]
        ...
        if move_infos:
            ...
            def _filter_moves(
                move_infos, threshold_base, chaos_relax, ownership_grid, opponent_coords, player_sign, best_score
            ):
            ...
            if complex_mode:
                ...
                complexity_weight_by_gtp = {
                    Move((x, y), player=self.cn.next_player).gtp(): w for (x, y), w in complexity_weights.items()
                }
            ...
            if not complex_mode and not good_moves:
                ...
                if not good_moves:
                    good_moves = _filter_moves(
                        move_infos, _FILTER_ABSOLUTE_CAP, 0.0, ownership_grid, opponent_coords, player_sign, best_score
                    )
                    if good_moves:
                        self.game.katrain.log(
            ...
            if is_area_scoring:
                ...
                if moves_without_pass:
                    ...
                    self.game.katrain.log(
                        ...
                        f"(better non-pass moves exist, best={best_gtp_by_score})",
                        OUTPUT_DEBUG,
                    )
                    ...
                    if best_gtp_by_score and best_gtp_by_score != "pass":
                        return (
                            Move.from_gtp(best_gtp_by_score, player=self.cn.next_player),
        ...
        if len(top5) >= 2 and move_infos:
            ...
            if is_policy_close or is_visits_reversal or is_mcts_nonprefer:
                ...
                if s1 is not None and s2 is not None and abs(s1 - s2) >= _TIEBREAK_SCORE_DIFF:
                    ...
                    trigger = (
                        "policy" if is_policy_close else ("visits_reversal" if is_visits_reversal else "mcts_nonprefer")
                    )
                    self.game.katrain.log(
        ...
        ai_thoughts = f"\n{top_str}\n\n{label}: played {move.gtp()} " f"({filtered_count} bad moves filtered)"
        return move, ai_thoughts

    def generate_weighted_coords(self, legal_policy_moves, policy_grid, size):
        self.game.katrain.log(f"[FightingStrategy] Generating fighting-based weights", OUTPUT_DEBUG)
        weighted_coords, ai_thoughts = generate_fighting_weights(self.settings, policy_grid, self.game, self.cn, size)
        self.game.katrain.log(
...
def _passes_complexity_gate(
    loss, base_threshold, relaxed_cap, score_stdev, sharpness_min, complexity_weight, max_complexity_weight, weight_frac
):
    """1手がフィルタを通過するか判定する。"""
...
def _compute_star_opening_targets(board_size, stones, ai_player, n):
    ...
    if n == 3:
        ...
        committed_viable = [
            empty_pts for ai_count, opp_count, empty_pts in line_stats if ai_count == max_ai and opp_count == 0
        ]
        if committed_viable:
            targets = set()
...
class HumanStyleStrategy(AIStrategy):
    ...
    def generate_move(self) -> Tuple[Move, str]:
        ...
        while not (clean_error or clean_analysis):
            ...
            if wait_count % 100 == 0:
                self.game.katrain.log(
                    f"[HumanStyleStrategy] Waiting for clean analysis ({wait_count/100:.1f}s)", OUTPUT_DEBUG
                )
            engine.check_alive(exception_if_dead=True)
        ...
        if clean_analysis and not clean_error:
            move_infos = clean_analysis.get("moveInfos", [])
            self.game.katrain.log(
                f"[HumanStyleStrategy] Using CLEAN moveInfos ({len(move_infos)} moves) for score filter", OUTPUT_DEBUG
            )
        else:
            move_infos = analysis.get("moveInfos", [])
            self.game.katrain.log(
        ...
        if move_infos:
            ...
            if best_gtp_by_score == "pass":
                ...
                return Move(None, player=self.cn.next_player), "Best move is pass, forcing pass."
            self.game.katrain.log(
                f"[HumanStyleStrategy] Move {current_move}: phase={'opening' if current_move < opening_boundary else 'normal'}, threshold={BAD_MOVE_THRESHOLD} (boundary={opening_boundary})",
                OUTPUT_DEBUG,
            )
            self.game.katrain.log(
                f"[HumanStyleStrategy] Best move score: {best_score:.1f} (player={self.cn.next_player}), filtering moves losing {BAD_MOVE_THRESHOLD}+ pts",
                OUTPUT_DEBUG,
            )
            for mi in move_infos:
                gtp_move = mi.get("move", "")
            ...
            self.game.katrain.log(
                f"[HumanStyleStrategy] {len(good_moves)} moves pass score filter out of {len(move_infos)} searched",
                OUTPUT_DEBUG,
            )
        ...
        self.game.katrain.log(
            f"[HumanStyleStrategy] {len(moves)} candidate moves ({filtered_count} filtered out)", OUTPUT_DEBUG
        )

        # If all moves were filtered, fall back to the best move from search
        if not moves:
            self.game.katrain.log(
                f"[HumanStyleStrategy] All human moves filtered, using best search move", OUTPUT_DEBUG
            )
            if move_infos:
        ...
        if self.settings.get("force_star_opening", False) and moves:
            target_stars = _compute_star_opening_targets(board_size, self.game.stones, self.cn.next_player, 2)
        ...
        if current_move >= endgame_threshold:
            ...
            self.game.katrain.log(
                f"[HumanStyleStrategy] Endgame (move {current_move} >= {endgame_threshold}): playing top humanPolicy move",
                OUTPUT_DEBUG,
            )
        ...
        if (
            self.settings.get("first_impression_deviation", False)
            ...
            if self.settings.get("first_impression_green_blend", False) and deviation_candidates and top_moves:
...
class DivergenceStrategy(AIStrategy):
    ...
    def generate_move(self) -> Tuple[Move, str]:
        ...
        if error or not analysis or "humanPolicy" not in analysis:
            self.game.katrain.log(f"[DivergenceStrategy] Stage1 failed, falling back to policy", OUTPUT_DEBUG)
            policy_move = self.cn.policy_ranking[0][1] if self.cn.policy_ranking else None
            if policy_move:
        ...
        if clean_analysis and not clean_error:
            ...
            move_infos = analysis.get("moveInfos", [])
            self.game.katrain.log(
                f"[DivergenceStrategy] Stage2 failed, using biased moveInfos " f"({len(move_infos)} moves)",
                OUTPUT_DEBUG,
            )

        # moveInfos が空の場合は humanPolicy 最上位手を返す
        if not move_infos:
            self.game.katrain.log(f"[DivergenceStrategy] No moveInfos, using top humanPolicy", OUTPUT_DEBUG)
        ...
        # order=0 の手がパスなら強制パス
        order0_mi = next((mi for mi in move_infos if mi.get("order", 999) == 0), move_infos[0])
        ...
        self.game.katrain.log(
            f"[DivergenceStrategy] {len(candidates)} candidates after score filter " f"(filter={score_filter})",
            OUTPUT_DEBUG,
        )
        ...
        chosen_order = next((ord_ for m2, _, _, ord_, _ in candidates if m2.gtp() == move.gtp()), "?")
        ai_thoughts = (
            f"\n{top5_str}\n\n"
...
class SiegeStrategy(AIStrategy):
    ...
    BOARD_PARAMS = {
        ...
        13: {
            "transition_move": 25,
            "min_group_size": 4,
            "concede_max_loss": 3.0,
            "max_loss": 4.0,
            "proximity_stddev": 2.5,
        },
    }
    ...
    def generate_move(self) -> Tuple[Move, str]:
        ...
        if in_attack_phase:
            ...
            self.game.katrain.log(
                f"[SiegeStrategy] Phase: {phase}, move={current_move}, targets={len(targets)}", OUTPUT_DEBUG
            )
    ...
    def _generate_concede(
        self, human_policy, move_infos, concede_max_loss, player_sign, best_score, best_gtp_by_score, is_area_scoring
    ):
        ...
        if len(top5) >= 2 and move_infos:
            ...
            if is_policy_close or is_visits_reversal or is_mcts_nonprefer:
                ...
                if s1 is not None and s2 is not None and abs(s1 - s2) >= _TIEBREAK_SCORE_DIFF:
                    ...
                    return (
                        winner,
                        f"Siege[concede] tiebreak({trigger}): played {winner.gtp()} (score diff={abs(s1-s2):.1f}pt).",
                    )
    ...
    def _generate_concede_fallback(self, candidate_moves, concede_max_loss):
        ...
        if not weighted_moves:
            self.game.katrain.log(f"[SiegeStrategy:concede] No valid moves, playing best move", OUTPUT_DEBUG)
            return (
                Move.from_gtp(candidate_moves[0]["move"], player=self.cn.next_player),
                "Concede fallback: no valid moves.",
            )
    ...
    def _generate_attack(
        ...
        if not moves:
            self.game.katrain.log(
                f"[SiegeStrategy:attack] No valid moves within {max_loss}pt, playing best", OUTPUT_DEBUG
            )
            if best_gtp_by_score and best_gtp_by_score != "pass":
                return (
                    Move.from_gtp(best_gtp_by_score, player=self.cn.next_player),
                    "Attack fallback: no moves within threshold.",
                )
            if move_infos:
                fb = move_infos[0].get("move", "pass")
                if fb == "pass":
    ...
    def _generate_attack_fallback(self, candidate_moves, targets, max_loss, proximity_stddev):
        ...
        top5 = heapq.nlargest(5, weighted_moves, key=lambda t: t[1])
        self.game.katrain.log(
            f"[SiegeStrategy:attack] Targets: {len(targets)}, candidates: {len(weighted_moves)}", OUTPUT_DEBUG
        )
...
class HuntStrategy(AIStrategy):
    ...
    def _select_final_move(
        ...
        ai_thoughts = f"\n{top_str}\n\n{phase_name}: played {move.gtp()} " f"({filtered_count} bad moves filtered)"
        return move, ai_thoughts
    ...
    def generate_move(self) -> Tuple[Move, str]:
        ...
        if clean_analysis and not clean_error:
            move_infos = clean_analysis.get("moveInfos", [])
            self.game.katrain.log(f"[HuntStrategy] Using clean moveInfos ({len(move_infos)} moves)", OUTPUT_DEBUG)
        else:
            move_infos = analysis.get("moveInfos", [])
            ...
            if focus_anchors:
                ...
                for i, (ax, ay) in enumerate(focus_anchors):
                    if i == 0 and self.cn.move and self.cn.move.coords:
                        ...
                        anchor_strs.append(
                            f"unstable({'group' if has_group_targets else 'invasion'}" f"({ax:.0f},{ay:.0f}))"
                        )
                self.game.katrain.log(
                    f"[HuntStrategy] Focus: anchors=[{','.join(anchor_strs)}] " f"stddev={hunt_focus_stddev}",
                    OUTPUT_DEBUG,
                )
        ...
        if has_group_targets:
            ...
            phase_name = "Invade"
            self.game.katrain.log(
                f"[HuntStrategy] Phase: Invade (invasion_targets={len(invasion_coords)}, " f"no group targets)",
                OUTPUT_DEBUG,
            )
        else:
        ...
        if hunt_dead_stone_avoid and moves and move_infos and self.cn.ownership:
            ...
            _own_stone_coords_dsa = {s.coords for s in self.game.stones if s.player == self.cn.next_player and s.coords}
            ...
            for i, (m, w) in enumerate(moves):
                ...
                if is_dead_zone_move(
                    ...
                    own_val = _ownership_grid_dsa[m.coords[1]][m.coords[0]] * player_sign if m.coords else 0.0
        ...
        # 最終選択（子クラスでオーバーライド可能）
        return self._select_final_move(
            moves,
            phase_name,
            move_infos,
            best_score,
