...
class TsumegoSolverStrategy(AIStrategy):
    ...
    def _solver_answer_rejected(self, coords, settings) -> bool:
        ...
        **全件がソルバ経路**で、13件すべて記録された正解手が KataGo の visits 順位0か1だった。
        13/13 が決定的に再現し、キャッシュを切っても変わらない（case AB のキャッシュ汚染では
        ない）。`solver_enabled=false` にすると 10/13 が手順まるごと正解に転じた。

        **キャッシュの追試は当時 `--no-solver-cache` で測っており、そのフラグは部分 no-op**
        （出題前検算しか cold にならず、手番ごとの solve は永続キャッシュを引いたまま）だった。
        修正後（2026-08-16）のハーネスで 13 件をケースごとに新規プロセスで cold/warm 対に
        走らせ直すと**判定 13/13 一致**で、キャッシュ汚染ではないという結論は変わらない
        （spec §9.5.1）。ただし「13/13 不一致」自体は今日のコードでは再現せず、**初手が正解に
        転じたのが 5/13・手順まるごと正解が 3/13**（キャッシュではなくソルバ側の変更ぶん）。
