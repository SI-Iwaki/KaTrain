...
| gain_rescue_margin | 1.0 | **ガード外救済**: 目数ガードで足切りされた候補でも、gain が選択手をこの値超えて上回り・深さゲート（visit比）を通り・同深さ検証で確定した場合だけ採用する。枠なし盤では「殺し損ねても外の空き地で取り返せる」ため目数差が圧縮され、正解の pointsLost がガード帯を挟んで揺れる（実測 case G2: C13 が 1.56〜2.26 でコイン投げ足切り→B13 誤答。gain 差は約10、同深さ検証 +8.4 で3run安定）。深さゲート通過後の偽 gain ≦0.7 に対し余裕。`gain_verify=false` なら救済ごと停止。枠ありでは実測9runで一度も発動せず不活性 |
