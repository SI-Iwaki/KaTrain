...
**失着の層（spec §13・既定 OFF）の計測**（2026-09-25・13路・blunder_max_loss 10・per_game 1・hp_ratio 0.7・`VEIL_BLUNDER_PROB` 0.5）: 影で条件を満たす手番は loose 0.45 回/局（spec の初期値 1.45 回/局＝支払い上限が低いほど多い）。ON（loose・通常の相手 20局）で 0.20 回/局（4/20 局）・深い読みの損失 6.8〜9.8 目・4局とも勝ちで、勝ち・flip・一致率は OFF と差が無い。接戦ストレス（段階3 の条件）では1回だけ（ほかは関門で止まる）・19-0-1。**ユーザーの決定（2026-09-25）: 条件は下限（hp が最善手の 0.7 倍以上）だけのまま・既定 OFF**。
**S13 の修正（2026-09-25）**: 失着 ON の接戦ストレスの持碁の1局は、失着ではなく決着局面の即決の読み落とし（生 0.36 目 → 実損 16.3 目・リード +15.8 → −0.5 目）が原因。S13 を、打つ前に best と候補の2手をクリーン 500v＋hp でプローブし `veil_decided_verified_ok`（vloss <= F_eff + 0.3・lead − vloss >= reserve・着手後勝率 >= min_winrate）を満たさなければ打たずに通常の流れへ進む形に直した。直した後の接戦ストレスは流していない（事件の形は `tests/test_ai_veil.py` の回帰テストで固定）。
