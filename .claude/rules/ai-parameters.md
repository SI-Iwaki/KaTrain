...
| `enigma9_gamble_until_move` | 13路と同じ機構（`enigma13_gamble_*` 参照）。**未校正**（目安は 13路の 35 を盤の点数比でスケールした 16） | OFF/8/10/12/16/20 | **0（OFF）** |
| `enigma9_gamble_min_winrate` | 同上 | 25/30/35/40/45% | **0.35** |
| `enigma9_gamble_min_delta_e` | 同上 | 0.3/0.5/0.7/1.0/1.5 | **0.5** |
...
| `enigma13_gamble_until_move` | **序盤の賭け罠（2026-09-17・spec `2026-09-17-enigma-gamble-design.md`）**。手数がこの値未満 × ヨセ前 × 消費モードでない（`cost_weight >= 1`）手番で、スコアリング済みの挑戦者に「`wr_after >= gamble_min_winrate`・`E − E_best >= gamble_min_delta_e`・`find_hp <= ENIGMA9_GAMBLE_MAX_FIND`(0.25)」を全部満たす手があれば net 比較・net_margin・局所性・難解＋の ΔE 床を通さず `u = ΔE − 0.5·max(0, vloss)`（`ENIGMA9_GAMBLE_COST_WEIGHT`）最大の手を打つ（純関数 `enigma9_gamble_pick`。rarity 項は資格にも順位にも使わない＝追記12 の「珍しさだけに払う外し」を避ける）。**cap は動かさない**（aim_jigo の頭打ち・通常の勝率フロアも先に効く）。窓の中は shortlist を spread +4 まで広げる（`ENIGMA9_GAMBLE_PROBE_EXTRA`・基底の安い順 7 手は互角の序盤で vloss 0.6 以下しか見ないため。難解＋で probe_extra >= 4 なら不変・基底はコールド約 +0.8 秒/手・先読み wave2 は同期しない）。0 = OFF＝採用判断・解析条件ともビット同一。難解＋も `_generate_move` を継承（同名キー）、擬態（13路）は上書きしているので対象外。**在庫の実測**（難解＋13路 18 局のログ再集計・手数<=35 の 215 手番・KataGo なし）: 13路序盤の勝率勾配は約 19%/目＝35% は互角から 0.8〜1.0 目損で、cap(1.6) と勝率フロア(25〜30%) は元々これを許している＝止めていたのは net 比較。起点のユーザー要望「応じられたら 35%・応じ損ねたら優勢」は**在庫なし**（着手後勝率 30〜45% の 293 候補で E>=1.0 は 1 手・E>=1.5 は 0。E>=2 の 80 手は全部 着手後も勝率 50% 以上＝消費モードが既に打っている）。緩和版の資格は非消費 116 手番中 16＝約 0.9 回/局（10 手番は資格 1 手だけ）、選ばれる手は平均 vloss 0.89・ΔE 0.71・着手後勝率 35〜69%、現行 net が同じ手を選ぶのは 4/16。**実局面**（ログ復元 SGF・katrain_debug）: `184025`@12 は OFF で G9（実戦と同じ）、ON 3 run で D5 / K4 / D5（資格の顔ぶれは run 間で入れ替わる＝find_hp 0.218 が境界 0.25 の近く）、`003547`@26 → H12（ΔE 1.31・vloss 1.02・wr 63%）、`221230`@10 → L6（ΔE 0.82・vloss 0.81・wr 36.5%）、同 @18 は資格なし → 従来の C8、`184025`@30/@40 は消費モードで `Gamble` 行なし、基底 enigma13 の校正局 @21 → B5。**実戦校正は未実施**（成功基準は 賭け罠の回数/局・打った後の相手の実損失〈ログの lead 差分〉・勝敗を OFF と並べて読む。勝率が落ちた回数だけで判定しない）。確認はログの `Gamble: window` / `Gamble: played` 行 | OFF/20/25/30/35/40/50 | **0（OFF）**・推奨 35 |
| `enigma13_gamble_min_winrate` | 賭け罠を打ったあと相手が最善で応じた場合の勝率（子局面プローブの検証値）の下限。通常の `min_winrate` も効く＝実効は大きいほう | 25/30/35/40/45% | **0.35** |
| `enigma13_gamble_min_delta_e` | 罠とみなす E の上積み（目）。0.3 にすると発火は 2 倍強だが、払う損失に対する E の回収が落ちる（ログ再集計: 0.5 で E−vloss 平均 −0.24 → 0.3 で −0.35） | 0.3/0.5/0.7/1.0/1.5 | **0.5** |
| `enigma13_overdraft_deficit` | **捨て身の罠（2026-09-18・spec `2026-09-18-enigma-overdraft-design.md`）の最大ビハインド D（目）**。ヨセ前 × 勝勢の消費モード（`cost_weight < 1`）の手番で、通常上限 cap の外側〜`over_cap = min(lead + D, ceiling)`（`ceiling = max(cap, 1.5 × large_lead_max_loss)`）の候補を（**天井まで払っても帯に届かない大差の手番＝`lead − ceiling >= min(target, answered_max)` では窓を閉じる**＝既定ではリード 12 目未満だけが対象・実戦 3 局で 96 窓中 55 が無駄打ちだったのを修正）生 loss の帯 `(cap − 1.5, over_cap + 1.5]` から `overdraft_probes` 手だけ追加プローブし（`enigma9_overdraft_probe_picks`＝trusted を loss 等間隔・足りなければ浅い候補を visits 順）、検証済み損失が cap を超えた候補（通常の shortlist 由来で上限超えだった手も破棄せず回す。**通常の候補プールが空＝最善手以外が全部 cap の外の一本道の局面でも、窓が開いていれば早期 return せず探す**＝実局面 `004112` d=50 の G10 がこの形）のうち「`vloss <= ceiling`・`−D <= lead_after < min(target, overdraft_answered_max)`・`find_hp <= 0.25`・`lead_after + e_fooled >= overdraft_min_fooled_lead`」を全部満たす手があれば、net 比較・net_margin・局所性・難解＋の ΔE 床・**勝率フロア**を通さず `lead_after + E` 最大の手を打つ（`enigma9_overdraft_pick`）。`e_fooled`＝十分でない応手（損失 > 0.3）だけの humanSL 重みつき平均損失（`enigma9_fooled_punish`）。0 = OFF＝ビット同一。序盤の賭け罠（`cost_weight >= 1`）とは排他、擬態（13路）は対象外。**実測**（13路 18 局の反実仮想・消費モード 182 手番・12 プローブ・`calibration-data/enigma-overdraft/`）: 既定（必ずマイナス・下限 2 目）で 1.28 回/局、6 プローブ換算 **0.82 回/局・55% の局で 1 回以上**。リード 3.5 目以下の 126 手番は在庫 0（＝消費モードに限る根拠）。選ばれる手は中央値 vloss 8.4・応じられたら −0.48（勝率 41%・最悪 30%）・引っかかれば +3.3・**E − vloss −4.25＝期待値では毎回 3〜4 目の損＝演出用**。「引っかかれば現リードを維持」まで満たす手は 0.06〜0.33 回/局。プローブ前に罠を見分ける特徴は無く発動回数はプローブ数にほぼ比例。確認はログの `Overdraft: window` / `Over <gtp>:` / `Overdraft: played`。**実戦校正は未実施** | OFF/1/1.5/2/2.5/3/4/5 | **0（OFF）**・推奨 2.5 |
...
| 他18項目（`enigma*plus_max_loss` 〜 `enigma*plus_opening_humanstyle_moves`・序盤の賭け罠 `enigma*plus_gamble_*` の 3 項目と捨て身の罠 `enigma*plus_overdraft_*` の 4 項目を含む） | 基底の難解（同じ盤）の同名項目と同じ意味・同じ候補値・同じ既定（テスト `test_base_options_are_shared_with_the_base_strategy` で固定）。ユーザーローカル config は基底の現在値を写してある（9路: max_loss 1.6・large 6.0・min_wr 0.25・target 0.2・aim_jigo true / 13路: max_loss 1.6・target 1.5 / 19路: max_loss 2.6・endgame 180・locality 3.0） | — | — |
...
| `enigma19_gamble_until_move` | 13路と同じ機構（`enigma13_gamble_*` 参照）。**未校正**（目安は 13路の 35 を盤の点数比でスケールした 75） | OFF/40/50/60/75/90/110 | **0（OFF）** |
| `enigma19_gamble_min_winrate` | 同上 | 25/30/35/40/45% | **0.35** |
| `enigma19_gamble_min_delta_e` | 同上 | 0.3/0.5/0.7/1.0/1.5 | **0.5** |
...
`game._veil_state["veil{9,13,19}"]`（endgame・close_drift・ledger・blunders＝今局の失着の数・forced（ON で打った数・打ったときだけ作る）・opening（序盤の窓で難解＋が最善手と違う手を打った数・打ったときだけ作る））、ログタグは `[Veil13Strategy]` 等。ponder と HumanStyle 委譲は使わない（序盤の研究外しで任せた難解＋13/19路の中の序盤委譲を除く）。
...
| `veil*_free_loss` | 同値外し（一致率に関係なく常に打つ外し）の損失の上限（目）。劣勢・一致率 15% 以下では 0.1 | 0〜0.5 | 0.2 / **0.4** / 0.3 |
...
| `veil*_spend_rate` | 損をする外し1回に使う余剰（リード − reserve）の割合 | 0.25〜1 | 0.5 / **1** / 0.5 |
| `veil*_max_loss` | 1手の支払い上限（目・ヨセ前）。罠の損失の上限も兼ねる | 9路 1〜5・13路 2〜6・19路 3〜8 | 3 / **6** / 6 |
| `veil*_yose_max_loss` | 1手の支払い上限（目・ヨセ）。0 で同値外しだけ | 0〜3 | 1 / **2** / 2 |
| `veil*_dominant_hp` | 9段が最善手を選ぶ確率がこれ以上＝明らかな一手（一致率が目標を超えているときだけ外す）。OFF で判定しない | 60%〜95%／OFF | 80% / **OFF** / 80% |
| `veil*_dominant_max_loss` | 明らかな一手で外すときの支払いの上限（目・通常の上限を絞るだけ） | 0.5〜4 | 1.5 / **3** / 3 |
| `veil*_min_human_policy` | 外す先に要る 9段の humanPolicy（絶対床） | 1%〜10% | 5% / **5%** / 5% |
| `veil*_natural_ratio` | 外す先に要る humanPolicy の第一感比（相対床・明らかな一手の局面では使わない） | 0.1〜0.5 | 0.2 / **0.1** / 0.2 |
...
| `veil*_blunder_mode` | 失着の層（spec §13）: 9段でも迷う局面（失着の手の 9段 hp >= max(15%, blunder_hp_ratio × 最善手の hp)）で、支払い上限（max_loss）を超える損失の手をまれに打つ。LOG は条件を満たす手を探して `Decision:` に書くだけ（影）、ON は資格のある手番の半分（`VEIL_BLUNDER_PROB`）で打つ。関門: ヨセでない・blunder_max_loss > max_loss・root 勝率 >= 95%・lead >= reserve + 5 + max_loss。資格: 深い読み（1500 visits）で max_loss < vloss <= blunder_max_loss・着手後リード（深い読みと root リード − vloss の両方）>= reserve + 5・着手後勝率 >= 95% | OFF/LOG/ON | OFF / **OFF** / OFF |
| `veil*_blunder_max_loss` | 失着の上限（深い読みの検証済み損失・目）。max_loss 以下なら失着は出ない（関門で止まる） | 9路 4〜8・13路 8〜15・19路 8〜20 | 6 / **10** / 15 |
| `veil*_blunder_per_game` | ON のとき1局で打つ失着の上限（LOG では数えない） | 1〜3 | 1 / **1** / 1 |
| `veil*_blunder_hp_ratio` | 失着の手の 9段 hp ÷ 最善手の hp の下限（9段自身が迷う局面の条件） | 50%〜100% | 70% / **70%** / 70% |
| `veil*_forced_mode` | 最善手しか無い手番の外し（spec §14）: 通常の流れが最善手で終わる4つの出口（no_pool / no_natural / no_shortlist / none_qualified）で、9段らしさの床以上の手を hp の高い順に最大 4 手（`VEIL_FORCED_PROBES`）500v で検証し、資格（cost = max(0, cons) <= 上限〈ヨセは yose_max_loss〉・lead_after >= forced_min_lead・lead − cost >= forced_min_lead・wr_after >= forced_min_winrate）のある手の hp 最大を、LOG は記録だけ・ON は打つ。関門: u > 0・root 勝率 >= forced_min_winrate・min(上限, lead − forced_min_lead) > 0。**ON のときだけ要件1を「20局に1局ほどの負けは許す」に緩める（ユーザーの決定 2026-09-25）**。失着の層と両方 ON のときは、失着の層が見送った手（抽選・1局の上限・失着の条件）も、この層の条件を満たせば打つ（失着の層の限度はこの層の手には効かない） | OFF/LOG/ON | OFF / **OFF** / OFF |
...
モジュール定数: `VEIL_BEHIND_LIMIT=-1.0`, `VEIL_BLUNDER_MARGIN=5.0`, `VEIL_BLUNDER_MIN_HP=0.15`, `VEIL_BLUNDER_MIN_WR=0.95`, `VEIL_BLUNDER_PROB=0.5`, `VEIL_BLUNDER_PROBES=2`, `VEIL_BLUNDER_RAW_MARGIN=2.0`, `VEIL_BLUNDER_VISITS=1500`, `VEIL_CLOSE_WR=0.9`, `VEIL_DECIDED_MARGIN=3.0`, `VEIL_DECIDED_WR=0.97`, `VEIL_FORCED_PROBES=4`, `VEIL_HP_TIE=0.02`, `VEIL_OPEN_DELEGATES={9: ai:enigma9plus, 13: ai:enigma13plus, 19: ai:enigma19plus}`, `VEIL_RAW_MARGIN=0.3`, `VEIL_STRICT_FREE=0.1`, `VEIL_TARGET_FLOOR=0.15`, `VEIL_TERMINAL_CLOSE_LEAD=3.0`, `VEIL_TERMINAL_CLOSE_MAX=0.05`, `VEIL_TERMINAL_MAX=0.1`, `VEIL_TERMINAL_MIN_VISITS=10`, `VEIL_TRAP_CREDIT=0.5`, `VEIL_TRAP_MIN_HP=0.02`, `VEIL_TRAP_PROBES=3`, `VEIL_TRAP_RAW_EXTRA=1.0`, `VEIL_TRAP_SWAP_MARGIN=0.3`, `VEIL_URGENCY_WIDTH=0.1`, `VEIL_YOSE_FREE_WR_DROP=0.01`。

**確認**: ログの `Rate:`（mine/n・opp/n_opp・p_match・u）/ `Endgame check:`（unsettled・max）/ `Budget:` / `Blunder…` / `Natural:` / `Decided:` / `Score …` / `Deviate:` /
`Terminal…` / `Decision: {json}`（tier i/ii/iii/terminal/blunder/forced/opening/failsafe・kind best/free/paid/trap/decided/swap/finish/pass/blunder/forced/opening・
why board / no_cands / pass / no_lead / no_pool / no_hp / dominant_closed / no_natural / no_shortlist / no_best_probe / none_qualified / invariant / exception / opp_pass / terminal / terminal_finish_rejected / opening / opening_best。失着の層が LOG／ON の手番はフィールド `blunder`＝gate / no_hp / no_cand / no_probe / rejected / shadow / skipped / played / invariant と、候補を選べたら `blunder_gtp` `blunder_vloss` `blunder_hp` `blunder_best_hp` `blunder_wr` `blunder_lead_after`。S13 の即決をプローブで退けた手番は `decided_rejected` `decided_vloss` `decided_wr`。最善手しか無い手番の外しが LOG／ON の手番はフィールド `forced`＝gate / no_hp / no_cand / no_probe / rejected / shadow / invariant / played と `forced_from`（元の why）、候補を選べたら `forced_gtp` `forced_cost` `forced_vloss` `forced_hp` `forced_wr` `forced_lead_after`。序盤の研究外しの窓の手番はフィールド `open`＝gate / invariant / played と `open_index`、任せた手番は `open_src` `open_in_cands` `open_secs` `open_thoughts` と `lead` `root_wr`（任せた先の難解＋の設定が aim_jigo ON の手番だけ `open_aim_jigo: true`・それ以外は無い）。窓の中の例外の手番は `open_index` だけ）。
CLI: `python -m katrain_debug --sgf <SGF> --move N --strategy veil9|veil13|veil19`。
**校正**: 13路は自己対局ハーネスで測定し（`docs/superpowers/specs/calibration-data/selfplay/veil13-campaign.md`）、**2026-09-25 のユーザーの決定で既定を段階1b の loose に変えた**（上の表の 13路の値。安全条件 reserve 5・min_winrate 0.85 は据え置き。spec §6.1 の表の 13路の列は設計時の初期値＝以下「spec の初期値」。9/19路は変えていない）。
段階1（5アーム × 20局・相手は humanSL 1k/1d/3d のプール）と段階3（接戦ストレス・下記）を実施し、2026-09-24〜25 にユーザーの指示で段階1b（安全条件以外のつまみ 9アーム）・2（投了なし）・3b（接戦ストレスの追加）も実施した。**段階1b: 安全条件を変えずに一致率を下げたのは loose だけ**（loose＝max_loss 6・spend_rate 1.0・dominant_max_loss 3・yose_max_loss 2・dominant_hp OFF・natural_ratio 0.1・free_loss 0.4、安全条件は据え置き）＝42.4%（同じ run の spec の初期値 49.0%・差 −6.5pt [t −11.3, −1.8]・損失 0.37 目/手・外した手の hp 中央値 23.7%）。単独のつまみ（dominant_hp 0.9 / OFF・natural_ratio 0.1・free_loss 0.4・close_drift_cap 1.0）は下げない（dominant_hp OFF は逆に +5.4pt）。spec の初期値の再実行は 53.1% → 49.0%（同じ設定・同じ seed の A/A で 4.1pt ずれる）。段階2: 投了なしでも spec の初期値は 47.4%（終盤 45.5%）で上がらない。段階3b: loose とユーザー設定（reserve 2・min_winrate 0.8・spend 0.75・max_loss 6・罠 ON）も接戦で 20-0（flip 0.05 / 0.20 局）＝(a) 可。失着の層の計測（下）で loose と spec の初期値を測り直した: 自分の一致率は loose 42.4% / 45.8% / 47.4%（段階1b・影・ON の OFF 側＝3 run 平均 **45.2%**）、spec の初期値 53.1% / 49.0% / 52.4%（段階1・1b・影＝平均 **51.5%**）、同じ run の中の差は −6.5pt / −6.7pt＝水準は「loose で 45% 前後」。
段階1の spec の初期値の自分の一致率 局平均 **53.1%**（相手 26.3%・20/20 勝ち・flip 0.05/局・損失 0.24 目/手）＝目標 30% に届かない。
...
段階3 は spec の初期値・段階3b は loose とユーザー設定の 20 対のハーネス測定で、攻めプリセットは接戦で測っていない。接戦では lead < reserve の手番で損をする外しをしないので、自分の一致率は局平均 70.5%（spec の初期値。loose は 65.4%）に上がる。
**失着の層（spec §13・既定 OFF）の計測**（2026-09-25・13路・blunder_max_loss 10・per_game 1・hp_ratio 0.7・`VEIL_BLUNDER_PROB` 0.5）: 影で条件を満たす手番は loose 0.45 回/局（spec の初期値 1.45 回/局＝支払い上限が低いほど多い）。ON（loose・通常の相手 20局）で 0.20 回/局（4/20 局）・深い読みの損失 6.8〜9.8 目・4局とも勝ちで、勝ち・flip・一致率は OFF と差が無い。接戦ストレス（段階3 の条件）では1回だけ（ほかの手番は関門で止まるか〈849〉、候補なし〈50〉・資格なし〈6〉で打たなかった）・19-0-1。**ユーザーの決定（2026-09-25）: 条件は下限（hp が最善手の 0.7 倍以上）だけのまま・既定 OFF**。
**最善手しか無い手番の外し（spec §14・既定 OFF）**: 2026-09-25 のユーザーの決定（20局に1局ほどの負けを許して一致率を平均 30〜45% に）で足した層。見積もり（反実仮想）はハーネス 36〜42%・実戦 43〜45%（13路・既定の forced 値）。9段らしさの床は変えないので、明らかな一手（9段 hp >= 0.95）は開かない（安全条件を全部外しても下限はハーネス 24%・実戦 28%）。計測（2026-09-25・ハーネス・失着 ON と比べた）: 通常の相手 20 対で 44.1% → 38.5%（同じ seed の差 −5.5pt・p 0.036）・20-0、接戦ストレス 40 対で 66.1% → 62.6%・40-0。この層の手は 5.65 回/局（接戦ストレス 3.67）・9段 hp 中央値 0.275・≥6目の手は 0.75 回/局（失着 ON だけのとき 0.35）。ハーネスの警報 nonfree_below_reserve は この層の手を数えるので kind 別に読む（この層以外は 0）。詳細は campaign md。
...
**S13 の修正（2026-09-25）**: 失着 ON の接戦ストレスの持碁の1局の直接の原因は、83 手目の決着局面の即決の読み落とし（生 0.36 目 → 実損 16.3 目・リード +15.8 → −0.5 目）。57 手目の失着（レポート 4.8 目・+23.6 → +18.7 目）が無ければ 83 手目の時点のリードは約 +20.6 目で、同じ読み落としでも約 +4 目残って勝っていた計算＝失着も重なった。S13 を、打つ前に best と候補の2手をクリーン 500v＋hp でプローブし `veil_decided_verified_ok`（vloss <= F_eff + 0.3・lead − vloss >= reserve・着手後勝率 >= min_winrate）を満たさなければ打たずに通常の流れへ進む形に直した。直した後の接戦ストレスは流していない（事件の形は `tests/test_ai_veil.py` の回帰テストで固定）。
