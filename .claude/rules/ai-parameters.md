...
| 他14項目（`enigma*plus_max_loss` 〜 `enigma*plus_opening_humanstyle_moves`・序盤の賭け罠 `enigma*plus_gamble_*` の 3 項目を含む） | 基底の難解（同じ盤）の同名項目と同じ意味・同じ候補値・同じ既定（テスト `test_base_options_are_shared_with_the_base_strategy` で固定）。ユーザーローカル config は基底の現在値を写してある（9路: max_loss 1.6・large 6.0・min_wr 0.25・target 0.2・aim_jigo true / 13路: max_loss 1.6・target 1.5 / 19路: max_loss 2.6・endgame 180・locality 3.0） | — | — |
