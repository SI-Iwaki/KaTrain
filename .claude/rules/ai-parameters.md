...
| gain_rescue_margin | 1.0 | **救済**: gain 争いに参加できなかった候補（目数ガード外＋深さゲート外＝非 contenders）のうち、gain が選択手をこの値超えて上回る手を同深さ検証にかけ、**検証も同じマージンで上回ったときだけ**採用する（トリガーと採用の両方に使う。採用側が通常の覆し 0.3 より厳しいのは、深さゲートを迂回するため偽 gain の候補も検証まで来るから）。visit比条件は課さない — 本物の比 0.21〜0.49 と偽の比 0.24〜0.36 は重なっていて比では分離できず、分離できるのは同深さ検証だけ（本物 +4.5/+8.4 で勝ち、偽 −0.24〜−0.58 で負ける）。実測 case G2（ガード帯コイン投げ）・case H（ゲート比0.49足切り）を救済し、偽トリガー（G1 の C13、F の N6）は検証が全run却下。`gain_verify=false` なら救済ごと停止 |
