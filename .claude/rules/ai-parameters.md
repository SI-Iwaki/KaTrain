...
`game._veil_state["veil{9,13,19}"]`（endgame・close_drift・ledger・blunders＝今局の失着の数・forced（ON で打った数・打ったときだけ作る））、ログタグは `[Veil13Strategy]` 等。ponder と HumanStyle 委譲は使わない。
...
`Terminal…` / `Decision: {json}`（tier i/ii/iii/terminal/blunder/forced/failsafe・kind best/free/paid/trap/decided/swap/finish/pass/blunder/forced・
why board / no_cands / pass / no_lead / no_pool / no_hp / dominant_closed / no_natural / no_shortlist / no_best_probe / none_qualified / invariant / exception / opp_pass / terminal / terminal_finish_rejected。失着の層が LOG／ON の手番はフィールド `blunder`＝gate / no_hp / no_cand / no_probe / rejected / shadow / skipped / played / invariant と、候補を選べたら `blunder_gtp` `blunder_vloss` `blunder_hp` `blunder_best_hp` `blunder_wr` `blunder_lead_after`。S13 の即決をプローブで退けた手番は `decided_rejected` `decided_vloss` `decided_wr`。最善手しか無い手番の外しが LOG／ON の手番はフィールド `forced`＝gate / no_hp / no_cand / no_probe / rejected / shadow / invariant / played と `forced_from`（元の why）、候補を選べたら `forced_gtp` `forced_cost` `forced_vloss` `forced_hp` `forced_wr` `forced_lead_after`）。
